...
| `gate_bypassed` | `tool`, `cwd`, `reason`, `why`: `admin_bypass` \| `user_grant` | a confirmation dialog was skipped by a standing hands-off switch: `admin_bypass` is the machine-wide admin switch (`tool_confirmation_bypass_admins`), `user_grant` is the per-user grant the admin made in the user record (`permissions["confirmation_bypass"]`). Emitted INSTEAD of `gate_required`, once per skipped dialog - hands-off never means unobserved. An authorizer's explicit `ask()` is never bypassed |
