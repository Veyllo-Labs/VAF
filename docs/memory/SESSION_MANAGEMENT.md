...
- **What counts as a still-existing chat:** the file is present (deletion is an `unlink` of `<id>.json`/`.json.gz`, there is no archived or paused state), `metadata.hidden_from_list` is not set, and - for surfaces that show chats - it is neither a channel session nor a thinking run (`list_ui`). Cross Chat Hint keeps channel sessions as sources but drops those whose endpoint belongs to a known contact.
