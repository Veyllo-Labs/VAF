...
When a contact with **Can reach your assistant** enabled sends a message (via WhatsApp, Telegram, a Discord direct message or, verified by the mail provider, by mail) and the Front Office door of that channel is open, the agent responds in **Front Office** mode: it uses a dedicated system prompt (role + anti-prompt-injection) and a restricted tool set so the contact cannot abuse the system. **The bot replies directly** to the contact on WhatsApp and Telegram, and on mail as a draft you approve first unless you switch the mail channel to send at once; the config key `front_office_contact_reply_require_approval` exists but is not read by any code at the moment (see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md), Reply approval). See [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md) for when it activates, which tools are allowed, and how to override the anti-injection text.
