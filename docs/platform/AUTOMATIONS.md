...
- **Web UI:** **Settings → Automations** lists and manages automations. The **Kalender** button in the main footer opens the calendar window: pick month, day and an hour slot in the automations lane to create a new automation (repeat, time, prompt, optional name); the events lane beside it holds the appointments of the VAF calendar (see [CALENDAR_INTEGRATION.md](../integrations/CALENDAR_INTEGRATION.md)). Creation is sent via WebSocket (`create_automation`); the list refreshes on success.
