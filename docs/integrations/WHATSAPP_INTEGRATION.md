...
- **Conversation header**: name, badge, number and a one-line explanation of what the agent does with this chat; **Add as contact** on a conversation or read-only chat creates a contact with "Can reach your assistant" (`POST /api/contacts`); when the number is already in the contact book (the WhatsApp sync creates such records without the flag), the header says so instead and shows a **Let the agent reply** switch: switching it on asks once whether the agent may talk to this person in Front Office mode and then sets the flag on the existing record, switching it off clears the flag without a question (`PATCH /api/contacts/{id}`; the dashboard payload carries `contact_id` and `contact_name` per chat). **Load older messages** is a pill at the top of the message list, where scrolling up lands, not a header button. The search over chats and contacts sits at the top of the chat list; the search field in the conversation header searches inside the open chat. The footer names the mode (full agent, Front Office, read-only) and the message count until the next Memory Learning.
