...
- **Contact with Front Office doesn't get a reply**:
  - The contact must have their **WhatsApp number** stored in the contact's **Channels** (phone/WhatsApp). If the contact has "Can reach your assistant" enabled but no WhatsApp channel, incoming messages from that number are **rejected** (not in whitelist). Add the contact's phone number in Settings → Connections → Contacts → edit contact → add Channel type "phone" or "WhatsApp" with the number (E.164, e.g. +4917642996812).
