...
- **Log Files** (collapsible) - lists every `.log` file in the VAF log directory (`~/.vaf/logs/`), grouped by domain (rag, memory, backend, prompt, headless, attach, tool_use, security, …) with a colour dot per domain. Collapsed by default. Most domains only fill while **Debug Logs** is enabled (on by default; toggle in **Settings → Advanced**, persisted as `debug_logs_enabled` in `~/.vaf/config.json`); the **security** domain (blocked access attempts, failed logins, rejected senders) is always written, independent of that setting. The switch is deliberately visible in the UI: the Logs page's empty states tell the user to "Enable Debug Logs", and a config with a legacy `false` would otherwise be a dead end (live incident on a macOS install).
