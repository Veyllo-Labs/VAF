...
- **The room is read INSIDE the ordinary chat area**, read-only. The chat's frame is
  ...
  A message that names files in the room's shared folder draws them as chips under
  the text (`body.files`, forwarded explicitly by the transcript projection - the
...
one window for every conversation across WhatsApp, Telegram, Discord, mail and agent rooms,
newest first, with the same states the channel windows and the agent's `inbox` tool show,
because all of them read the rows `vaf/core/inbox.py` builds. Three panes: the rail (views,
...
write a draft, which opens the channel window and starts the Composer there; "mark all as
read", above the list's search field next to the refresh symbol, reads the selected
channel (or all of them) at once, whatever the view or the search shows; the inbox
...
- A left **rail** with one dot per action - solid black = thinking, **hollow black ring = an intermediate spoken line (`'say'`)**, hollow gray ring = a running/failed tool, solid gray = a completed tool - grows down as steps arrive. The living **white-dot avatar** walks down to the active (running) step and returns to the top when the group collapses; a `'say'` step is always "done" so it never steals the avatar. The timeline's avatar renders with `noScene`: the wide tool/away scenes (magnifier, globe, ...) are for the free-standing loading bubble and would be clipped under the cards to the rail's right (their lean also detaches the dot from its rail); a running tool shows as in-place motion on the walking dot instead.
...
- **What a turn is, and what is not part of one.** A turn is the span between two USER messages, and its LAST assistant message supplies the visible answer. A **proactive** message (one carrying a `kind`, see below) belongs to no turn: it has no user message in front of it, so it would fall into the preceding turn, become its last assistant, and take over the answer bubble - the reply the user had already read would be folded away as a `'say'` step in the collapsed rail. The grouping therefore skips `kind`-tagged messages in all three of its passes (counting the turn's assistants and tools, building the rail steps, marking rows as consumed), so such a message renders as its own row with its own timestamp and avatar. This is the transcript half of the `agent_message_append` contract in [WEBUI_WEBSOCKET_FLOW.md](WEBUI_WEBSOCKET_FLOW.md); both halves are pinned by `tests/test_proactive_bubbles_stand_alone.py`.
...
**Settings → Advanced:** **Failover** (the resilience level and the provider chain behind it, plus the first-token deadline, the triggers, whether the primary is preferred again, and how long a provider that just failed is skipped before the next message re-checks it; all keys admin-only, see the Failover table in [CONFIG_SCHEMA.md](../setup/CONFIG_SCHEMA.md)), **Attachments** (hierarchical document indexing), system options - including the tool-step budget (`max_tool_turns_per_step`, with a no-limit switch `tool_loop_unlimited`, both admin-only) and the admin's own hands-off switch (skips tool confirmations for the admin's account, announced per use as a `gate_bypassed` event) - and management panels (each a row that opens a sub-modal): **Tools**, **MCP**, and **Workflows**. The **MCP** row ("N connected / M configured") opens a panel to add, edit, or remove MCP servers (admin only) - see [MCP_INTEGRATION.md](../agents/MCP_INTEGRATION.md). The last row, **Update and Repair**, opens the maintenance dialog described below.

**The Tools panel** (the sub-modal behind the **Tools** row) groups the installed tools into bundles - one card per integration or area, from the `category` each tool declares on its class (see [TOOL_ROUTER_ARCHITECTURE.md](../agents/TOOL_ROUTER_ARCHITECTURE.md), section 9). Clicking a bundle card opens a shelf across the full width of the grid, in its own row directly under the card that owns it, with that bundle's tool cards inside; a small notch on the card points at the shelf it opened. Only one shelf is open at a time. The grid itself does not reflow - the cards keep their places and the only movement is that what sits below the shelf makes room, which is why opening a bundle no longer sends a hundred unrelated cards to new positions. Tools a user uploaded appear in their own bundles - a custom tool declaring the GitHub bundle is listed under **Custom GitHub**, directly beside the built-in **GitHub** and never inside it, because a bundle says where a tool comes from and an uploaded file must not borrow the standing of the shipped ones. Typing in the search box switches the grid to the matching **tools**, flat and ungrouped, because someone searching is looking for a tool rather than for a bundle. The shelf's open and close durations grow with its height, so a bundle four card rows tall does not collapse four times as fast as a one-row bundle. Its header answers "how much of this does the agent already know" before you search for anything. Three counters sit to the right of the search box: **Total**, **Learned** and **Unlearned**, counted over the full tool list and not over the filtered grid, so they keep agreeing with the "N modules installed" line above instead of tracking what you typed. **Learned** is strict equality on the `learned` state and **Unlearned** is the remainder, so the two always add up to Total: the backend annotator can leave `learned_state` off a tool entirely, and an invalidated record (`stale`) is no longer counted as learned until the tool is trained again, so both land on the Unlearned side. The per-tool badge on each card is finer-grained and unaffected - it still reads `Learned`, `Learning`, `Stale` or `Not learned` for that one tool.

**While a training run is in flight**, the same header shows a centred amber pill, "Active learning run for `<tool>`", with a pulsing dot and a `+N` suffix when more than one run is going; clicking it opens that tool's training dashboard. It is fed by `GET /api/whare_wananga/active_runs`, which consumes the framework's own set-level reader (`vaf.whare_wananga.active_runs`) rather than filtering the job table a second time, and it is polled every 2 s while the modal is open instead of being pushed over the WebSocket: the indicator lives inside a modal a reader deliberately opened, not in a machine-wide banner, and it is invisible almost all of the time. Runs that nobody started from this browser light it up too - the eager worker, the retrain drain and the teacher all register there - but only inside the server process, so a `vaf ww train` started in a separate shell stays invisible.

**Training a tool** is offered from the code viewer (**7. Code Viewer** below), whose header carries exactly one of three mutually exclusive actions: a red **Tool not configured** label - not a button, the connection has to be set up first; a green **Tool trained** button that only opens the training dashboard to read the metrics and starts nothing; and an amber **Train tool now** button for every other state. The amber one asks first. The dialog states the two things a run actually costs: it runs the tool for real dozens of times and makes dozens of model calls to learn from the results, which count towards usage like any other call, and a started run cannot be stopped and has no time limit, so an interrupted run leaves the tool back at Learning until it is trained again. It deliberately does **not** claim that training blocks anything else, because it does not: a run refuses only a second run of the same tool, and automations, sub-agents and ordinary chat keep going beside it. Irreversible tools are never probed - the runner refuses every tool declaring `side_effect_class = "irreversible"`, every send lane among them, with `skipped=True` before it calls anything - so a training run cannot send a message.
...
*What happens during an update.* The endpoint spawns a DETACHED `vaf update` process, answers immediately with the server-clock `started_at` of the run, because the update stops the very server that answered, and the dialog waits on TWO voices. The first is same-origin: `GET /api/version` through the Next.js proxy every 2.5 seconds with a 2 second timeout; a version higher than the one it started from means success, and the page reloads itself three seconds later. But the updater invalidates the web build (`web/.next/BUILD_ID`), so after the restart the proxy stays down for its own `npm run build` for minutes after the backend is already back - which is why the second voice exists: whenever same-origin does not answer and the page runs on port 3000, the dialog also polls the backend origin (port 8001, `getApiDirectBase()`) directly. That endpoint is unauthenticated (the auth cookie is SameSite=Lax and never crosses origins), so the direct poll can learn exactly one thing, and it is the thing that matters: the NEW version answering there means the update itself succeeded and only the interface is still rebuilding (the waiting screen says so instead of spinning blind); the OLD version answering there after the port was down at least twice in a row smells like a rollback, and the waiting text says that too. The hard verdict needs the outcome file: `vaf update` records how every run ended in `~/.vaf/update_result.json` (`succeeded`, `rolled_back`, `recover_needed`, or `failed` for aborts that changed nothing, e.g. a dirty tree), `GET /api/system/update` serves it as `last_result`, and when same-origin answers with the OLD version the dialog reads it (authenticated, so same-origin only), accepts it only if its `finished_at` is at or after the run's `started_at`, and shows a failure screen naming what happened and where the log is (`~/.vaf/logs/update_web.log`) instead of waiting out the clock. No fresh outcome plus the old version simply means the updater has not stopped the service yet: keep waiting. After ten minutes it stops guessing; the timeout screen distinguishes "the update finished but the interface has not come back" (direct poll saw the new version) from "the server has not come back at all", and Keep waiting carries the run's freshness anchors along so a late verdict is still recognised. Two `sessionStorage` keys carry this across the reload: `vaf_update_pending` (written BEFORE the POST, so closing and reopening the dialog resumes the waiting screen; it also stores `started_at`) and `vaf_update_done` (the one-shot "update finished" line). The dialog does nothing to the WebSocket; `page.tsx` reconnects on its own and the final reload re-initialises everything. Six situations refuse with a `409` and a sentence instead of a broken restart: a package (pip) install; a source tree that is not a git checkout (a downloaded archive - `vaf update` can adopt it, but only after a terminal prompt that explains what `git reset --hard` does to the folder, and an unattended run would answer that prompt for the user); server mode without `systemd-run`; an earlier update that never finished; a repair still running; and a VAF the updater cannot identify at all (no service pid record, no instance record, nothing owning the tray's singleton port: the stop step would do nothing and the start step would add a second server). A desktop launch is fine on every platform: the running VAF records how it was started (`~/.vaf/instance.json`, see `vaf/core/instance.py`), and the updater stops that process and starts it again the same way, window and tray icon included. A second click while an update is already spawned is refused too, because both processes would fetch and reset the same checkout against each other.
...
**Settings → Connections:** Manage external integrations (Email, Calendar, Cloud, Discord, Telegram, WhatsApp, GitHub, etc.). The WhatsApp, Telegram and Discord windows and the mail window show the inbox's state on every row (the red unread pill, the amber "Waits for you" chip, "Agent answered", "Done"), turn the list header's right side into an "N waiting for you" button that jumps to the next waiting chat, and mark a chat read when it is opened; the three messenger windows also carry an "All read" button above their chat list, next to the refresh symbol, that reads every chat of that channel at once (see [INBOX.md](../integrations/INBOX.md)). A search field at the top filters the list by name or category (e.g. type “GitHub” or “Kalender” to jump to that connection). The **GitHub** category opens a dashboard with a rights-overview strip (toggle read/write per account), connected accounts, an event timeline (agent actions, newest first), and a repositories panel (repos for the selected account with links to GitHub). The **Calendar** category shows Google Calendar and Microsoft Outlook, the sync sources of the VAF calendar; the gear on a connected card opens the Calendar Dashboard, the sync settings (accounts with their sync state, the switch per account, which account new events go to, the default reminder, sync now); they use the same OAuth connection as Email (connect Gmail or Outlook under Email first). The **Contacts** gear opens the Contacts window, a wide two-pane contact book with a filterable list, bulk actions, one activity timeline per person (messages, mails, notes, events) and a jump into the WhatsApp or Telegram chat; fields, routes and the window are described in [CONNECTIONS.md](../integrations/CONNECTIONS.md#contacts). The **Inbound** card under Contacts (admin only; the Front Office) opens the Inbound window: a switch per channel, WhatsApp, Telegram and E-Mail, that lets everyone who writes there reach the agent (switching on asks once and allows the channel's contacts in the book; a person is kept out with their own "Let the agent reply" switch; the mail row adds the reply mode, draft first or send at once, and answers only senders the mail provider verified, see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md#mail)), the owner's instructions for those answers, and the knowledge, documents learned into a lane of the memory store that only a contact's turn reads, with the switch that lets those turns also read the owner's general memory; its header has **Open inbox**, and the inbox's rail has **Inbound** for the way over, see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md#the-front-office-window-in-connections). The **WhatsApp** window shows your agent's number's chats; where the agent does not answer (a read-only sender, or the channel with inbound to the agent off) it has a compose box under the conversation that sends your own words from that number and, to the right, the Composer that drafts into the box (the mail window's assistant on the shared lane), see [WHATSAPP_INTEGRATION.md](../integrations/WHATSAPP_INTEGRATION.md#dashboard). The **Email** tile opens the mail client window (three-pane; its gear opens the in-client account panel). Suspicious emails are marked there with a warning badge and reason; they remain visible to the user, but are hidden from agent mail tools by default for safety. Every thread row and reader header also carries the sender-verification badge (a green shield for a sender the account's own provider verified, a grey one for a pass on another domain, a warning shield for an unverified sender, red when the mail claims your own domain; machine mail such as a bounce or an auto-reply shows its kind instead; nothing until the provider's id is learned), and the account panel shows per account which provider id the verification trusts, with **Learn from the mailbox** to set it up, see [EMAIL_CLIENT.md](../integrations/EMAIL_CLIENT.md#verification-and-cases). When Inbound answers on mail, the agent's held answer appears above the thread's messages as "Your agent's draft" with **Send**, **Edit** (the reply composer opens with the text) and **Discard**; the inbox shows the same draft on the thread's row with Send and Discard. For safety, IMAP/SMTP hosts that resolve to loopback or a private/link-local address are refused (SSRF guard); operators running a legitimate LAN or self-hosted mail server can opt in by setting `email_allow_private_hosts: true` in `config.json` (default `false`). When a calendar is connected, the settings (gear) icon opens the **Calendar Dashboard**: left sidebar lists connected accounts with links to open Google Calendar or Outlook in the browser; main area shows upcoming events from the API with selectable range and refresh. See [CONNECTIONS.md](../integrations/CONNECTIONS.md) and [CALENDAR_INTEGRATION.md](../integrations/CALENDAR_INTEGRATION.md).

**Settings → Automations:** View scheduled automations (user-scoped when multi-user is used; root/global automations such as "Daily calendar check" are also shown so the list matches the agent's `list_automations` tool). To create one manually: click **Create New** (or use the **Kalender** entry in the sidebar footer) to open the calendar window; choose month, then day, then an hour slot in the automations lane. The window shows the appointments of the VAF calendar beside the automations: a day view with an all-day row, a sticky header naming the two lanes, an events lane (a click on an empty hour creates an appointment; the popup edits title, time, all-day, location, description, linked contacts, the reminder and whether the event is also written into the connected calendar) and the grey-tinted automations lane with lightning chips; the current hour is framed in red with the time under its label, and a legend above the month grid names the blue and grey dots (see [CALENDAR_INTEGRATION.md](../integrations/CALENDAR_INTEGRATION.md)). The sidebar footer also includes the **Inbox** (every conversation across every channel, with a badge: the amber count of conversations waiting for you, or a red dot when something is unread; see the Inbox window section and [INBOX.md](../integrations/INBOX.md)), **Notifications** (opens the Notifications popup) and **Settings**. Opening the calendar window (footer) also triggers the calendar ensure-daily-check API when a calendar is connected, so the Daily calendar check appears in the list without opening Settings first. A popup lets you set repeat (once, daily, weekly, monthly, hourly), time, a detailed prompt, and an optional name. Creation is sent via WebSocket (`create_automation`); the list refreshes on success. The agent can also create automations via the `create_automation` tool in chat.

The same calendar window includes a **per-user planner**:
...
| `GET` | `/api/security/events?date=YYYY-MM-DD&limit=100[&module=firewall]` | Structured security events for a day; `module=firewall` narrows to the firewall counter's kinds |
...
**Features:** Syntax highlighting for 40+ languages, live refresh every 2 s while the agent is generating (shown by a pulsing **LIVE** badge), in-browser editing with undo and redo (Ctrl+Z and Ctrl+Y or Ctrl+Shift+Z inside the editor, or the two arrow buttons in the header, which trigger Monaco's own history and are greyed out when it has nothing to take back; they are hidden while a Markdown file shows its rendered preview, because no editor is mounted then), and save back to disk via `POST /api/file/save` (Ctrl+S supported). The header shows the detected language, filename, save state, and last-updated time. The footer shows the full file path and unsaved-changes indicator. The custom tool editor in the Tools panel is the same Monaco editor and carries the same two buttons above its code area.
...
**A question with options (`AskChoices`):** when the agent asks with `ask_user` in a chat, its turn ends at the question (vaf/tools/ask_user.py, [AGENT_LOOP.md](../agents/AGENT_LOOP.md)). The turn's answer is the question followed by the options as a numbered list, which every surface without buttons shows as it is; the chat cuts that list off where the tool put it (whole lines, right after its own question, so an option's words inside a sentence stay) and draws the options as buttons under the answer (`web/components/chat/AskChoices.tsx`, the contract in `askContract.ts`: the tool result's `ASKED THE USER.` line and its JSON, pinned against the Python side by `tests/test_ask_user_chat.py`). A click sends the option as the person's next message, exactly as if typed, with the hint "pick an answer or type your own" under the buttons. Once the chat has a reply after that turn, the buttons stay as the record: the picked option dark with a check, the others dimmed; an answer in the person's own words dims them all. Both halves come from the history, so a reload shows the same.
...
**Undo and redo:** both editor paths have Undo and Redo as the first two toolbar buttons and answer Ctrl+Z, Ctrl+Y and Ctrl+Shift+Z (Cmd on a Mac) while the keyboard is inside the editor. The history is per open editor: switching chats reopens the editor without it. In the legacy HTML editor the history is the browser's own for the editable body. The buttons run `execCommand('undo')` and `execCommand('redo')` through the same lane as the formatting buttons and are greyed out from `queryCommandEnabled`; a rewrite of the body from outside (a load, or the agent's `replace_editor_selection` / `replace_editor_text`) starts a fresh history, because the sheet is written anew. The React state follows the sheet through every step: the effect that writes the sheet attaches the body's input listener on every run, not only on a rewrite. Before, the cleanup of the previous run removed the listener and the next run, which only had to consume the "this came from the sheet" flag, returned without attaching it again, so from the second keystroke on nothing typed reached the state that Save and the agent context read until a toolbar button re-read the body (live incident). In the native DOCX editor the history is VAF's own over the document model; what counts as one step is in [DOCUMENT_EDITOR_NATIVE_DOCX.md](../documents/DOCUMENT_EDITOR_NATIVE_DOCX.md) under "Undo And Redo".
...
`confirmed` is the browser reporting that a person read the delete dialog and
pressed its button. Without it the request is a permission question, not a
delete: the server checks the record itself and, for a chat that holds
anything, answers `session_delete_result` with `needsConfirm` instead of
removing it. `archive` keeps a copy (see the archive section above);
`hide_session` takes the same two fields, because a thinking run leaves the
...
```json
{
  ...
  "sessions": [
    {"id": "uuid", "title": "Session Name", "date": "ISO timestamp",
     "messageCount": 12, "source": null, "kind": "chat"}
  ]
}
```

Every row comes from one projection (`session_list_payload` in
`vaf/core/session.py`), and a room row carries more (`roomId`, `roomKind`,
`role`, `unread`, `members`, `closed`). `kind` is what keeps a room out of the
session loader. A hand-built row that dropped these fields is not hypothetical:
it is why the CLI's own push once made every chat in the sidebar look empty.
...
The answer to `delete_session` / `hide_session`. `needsConfirm` means the server
refused because the chat holds something and nobody confirmed: the browser opens
the delete dialog and asks again with `confirmed`. `deleted` means it is gone
(archiving counts, the file moved). Both false means nothing was removed, for
instance because another tab got there first. It is addressed by `id`, never
`sessionId`, so the socket's per-session filter cannot swallow it while a
different chat is open.
...
- **The column holds the position it was put in.** A `requestAnimationFrame` loop
  remembers the intended `scrollTop` and restores it whenever the view moved without a
  person having touched the wheel, a key or the surface. This exists because the engine
  moves the view on its own: measured in the tray window (WKWebView), every keystroke
  with an image in the conversation moved it from 1689 to 969, with content height,
  visible height, image reloads, scroll anchoring and our own writes all ruled out one
  by one. A real interaction with the scroller updates the intent instead, so the loop
  never fights the reader.
- **Therefore every programmatic scroll goes through `setChatScroll`** (declared beside
  `containerRef` in `web/app/page.tsx`). It clamps the target to the real maximum,
  writes `scrollTop`, declares that position as the intent and updates `isAtBottomRef`
  from the same threshold the scroll listener uses. A site that only assigns `scrollTop`
  scrolls and is put back within a frame, which on screen is a control that does
  nothing: this is what silently broke the prompt navigator, the session-switch scroll
  restore and the sub-agent panel's height preservation. `tests/test_chat_autoscroll_guard.py`
  fails when any chat-container `scrollTop` write does not declare the intent.
- **Never `scrollIntoView` on this container.** Measured in the desktop window against
  it, `behavior:'smooth'` moves it 0px, and where a smooth animation does move it, the
  position guard reverts it frame by frame. `scrollChatToMessage(trueIdx)` is the
  supported way to bring one message into view; it addresses the bubble by the
  `data-msg-idx` attribute, which carries the index in the FULL `messages` array, and
  returns `false` when that bubble is not rendered.
- **Scroll anchoring is off** for `.vaf-chat-col` and its children (`globals.css`), so
  the browser does not also adjust the position when content around it resizes.

### Prompt Navigator

- **What it is**: a column of small dashes on the right edge of the chat, one per recent
  user prompt (the last 9). Hovering opens the full list of that chat's prompts, newest
  at the bottom, and clicking a row jumps the conversation to that message.
- **Desktop only** (`max-md:hidden`): a rail on the screen edge sits under the thumb and
  eats the scroll gesture, see [MOBILE_UI.md](MOBILE_UI.md).
- **It reads `filteredMessages`, not `messages`**: a prompt the chat filters out (a
  `__CMD__` line, a thinking-mode preamble) has no bubble on screen, so a row for it
  would be a target that cannot be reached.
- **Widening the window is counted in turns.** The chat renders a virtual window of the
  last `MSG_TURNS + msgOffset` user turns, so a prompt older than that has no bubble to
  jump to; the click raises `msgOffset` to exactly the turn it needs. The jump is then
  parked in a ref and run by an effect that watches the widened window, not by a timer
  waiting a fixed number of milliseconds for a render it cannot observe.
...
- **Conversion ledger.** Converted: the chat's workspace explorer (five levels, see the Session Workspace Window section), the context window, the sub-agent palette, `ConfirmDialog`, the channel dashboards (`ChannelDashboardShell`, shared by WhatsApp, Telegram and Discord) and `ContactsDashboard` (four rungs keyed on their own state: the window at 50, the actions menu at 51, an open inline editor at 52, the edit form at 60, with its `ConfirmDialog` at 70). and `AutomationCalendarModal` (the calendar window: the window at 60, the note, to-do and automation-slot popups at 71, the event popup at 72, its `ConfirmDialog` at 80). Not converted yet: `SettingsModal`'s 22-branch stacked handler and its children (the largest site, and it carries real product decisions such as whether Escape discards an unsaved custom tool), `NotificationsModal`, `CreateAutomationPopup` (the other half of the calendar pair; it still binds its own listener, which the registry's capture-phase listener pre-empts while the window is up), `CodeViewer` (its Escape discards unsaved edits with no dirty check, which is a decision, not a mechanical move), `GitHubDashboard`, `SubAgentWindow`'s idle context menu. The ledger covers surfaces that bind Escape at all; one that binds nothing is simply not in it, and stays unaffected because an unregistered overlay behaves exactly as it did before the registry existed. A surface that must NOT be dismissed by Escape while it is up does not stay unregistered either: it registers with `onEscape: null`, which swallows the press so the layer underneath cannot answer in its place. The set that still hand-rolls a listener is a ratchet pinned by `tests/test_escape_answers_one_layer.py`.
...
- **Buttons**: the emphasis is deliberately inverted against the usual primary/secondary pair - the safe answer carries the emphasis fill and the confirming answer is the black button - because the confirmed action costs real work. A question whose answer deletes or removes something passes `destructive`, and the confirming button is red in both themes. The dark-mode mechanics are in [DARKMODE.md](DARKMODE.md).
- **Reach**: the "Train tool now" confirmation, the WhatsApp window's "let the agent reply" switch, and the Contacts window (delete a contact, delete the selection, remove a note or an event, all `destructive`; enabling Front Office for a person, not), and the Front Office window in Connections (switching a channel on, not `destructive`; removing a learned document, `destructive`). The rest of the app still confirms ad hoc through the browser's `window.confirm()` or a hand-rolled card; converting those is a separate sweep.
...
- **Auto-Scroll**: the position is set directly, never animated (see Chat Scroll Position above); a smooth scroll moves this container 0px and would in any case be retargeted by the next stream chunk 80ms later
