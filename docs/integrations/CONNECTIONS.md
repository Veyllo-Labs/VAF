...
- **Answer read-only chats yourself**: a chat the agent does not answer (a sender nobody allowed, or the whole channel with `inbound_to_agent` off) has a compose box under the conversation that sends from the agent's number, and the Composer beside it drafts into that box in the tone this number writes in. Your own message opens no reply window for the agent. Details in [WHATSAPP_INTEGRATION.md](WHATSAPP_INTEGRATION.md#dashboard).
