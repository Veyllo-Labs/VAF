...
# `vaf start` spawn `-m vaf.main tray`), and all subagent children (`-m vaf.main
# subagent run`). Embedders import only `vaf` and never reach this line: unregistered =
# unrestricted is the documented library default.
# Deliberately OUTSIDE bootstrap(): its early returns (VAF_SKIP_DEP_CHECK, missing
# requirements.txt on pip installs) would silently skip registration exactly where
# enforcement matters. No try/except: a failed registration must crash the process, not
# degrade every tenant to "unrestricted". Wiring pinned by
