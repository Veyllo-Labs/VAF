...
| `store_credential` | system | Store a password, token or login the person gave in the chat under a NAME in their own credential store. The argument is declared secret (`secret_args`): never logged, and after the call removed from everywhere VAF keeps the chat ([CONNECTIONS.md](../integrations/CONNECTIONS.md)). No confirmation and no plan gate: every round the value waits, it travels with the conversation. |
