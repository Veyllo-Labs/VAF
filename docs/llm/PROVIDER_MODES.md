...
| **All API (coder endpoint map)** | the coder talks raw OpenAI wire format and resolves its endpoint from its own map `coder_api_providers()` — it must cover EVERY provider in `PROVIDER_MODELS` (CI-enforced by `tests/test_coder_provider_map.py`); an unknown API provider errors loudly instead of falling back to the local `:8080` branch, so a misconfigured API provider fails visibly rather than being served by the local model | `vaf/tools/coder.py` (`coder_api_providers`, run() gate) | [CODER_ARCHITECTURE.md](../agents/CODER_ARCHITECTURE.md) |
