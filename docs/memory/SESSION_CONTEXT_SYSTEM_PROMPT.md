...
This block is built using `vaf/core/messaging_connections.get_messaging_connections(username, user_scope_id)` and is only shown when the list of available channels is non-empty. See [CONNECTIONS.md](../integrations/CONNECTIONS.md) and [USER_IDENTITY.md](USER_IDENTITY.md) for proactive messaging and `main_messenger`.
