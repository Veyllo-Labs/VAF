...
- **Source**: the session files under `~/.vaf/sessions` AND the user's agent rooms, read at query time. Nothing is indexed, cached or written, which is also what makes the deleted-chat guarantee structural: deletion is an unlink, so a deleted chat is unreadable rather than filtered out.
