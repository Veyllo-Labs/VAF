...
- **Contents**: `name`, `preferred_language`, `preferences` (list of strings), `dos` (list), `donts` (list), `change_log` (list of `{ "at": "<ISO8601>", "action": "<summary>" }`).
...
- **When to use**: When the user says their name, language, or rules (e.g. "call me Mert", "I prefer German", "always be concise", "don't use emojis").

Parameters (all optional): `name`, `language`, `add_preference` / `remove_preference`, `add_do` / `remove_do`, `add_dont` / `remove_dont`. At least one must be provided.
...
- **Left**: Contents of `user_identity.json` (name, language, preferences, do's, don'ts). Read-only; updates happen via the tool or when the user tells the agent.
