...
| `python_exec` | dangerous | Run Python on the host (no sandbox) - confirmed; the tool re-checks on its own (a workflow step runs without the gate) that the person allowed it beyond one call: a persisted `set_tool_policy("python_exec", "allow")` or their "for this chat" grant for this chat. "Only this time" is not enough. See the host-execution line in [EMBEDDING.md](../EMBEDDING.md). |
