...
| Platform | Status | Description |
|----------|--------|-------------|
| Google Calendar | Available | Sync source of the VAF calendar: its events are pulled in, VAF's events written back (uses Gmail OAuth) |
| Microsoft Outlook | Available | Sync source of the VAF calendar: Outlook/Microsoft 365 calendar both ways (uses Outlook OAuth) |
...
VAF has a calendar of its own (the calendar window in the sidebar footer; design in [CALENDAR_INTEGRATION.md](CALENDAR_INTEGRATION.md)), and the agent's calendar tools work on it without any connection. **Setup of a sync source:** Calendar uses the same OAuth connection as Email. Connect Gmail or Outlook in **Settings → Connections → Email**; the account is then synced both ways every few minutes, the gear on its card opens the Calendar Dashboard with the sync settings, and the agent lists, creates, updates and deletes events via the tools `list_calendar_events`, `create_calendar_event`, `update_calendar_event`, `delete_calendar_event`. In Google Cloud Console, enable the **Google Calendar API** for your project; the redirect URI is the same as for email (`/api/email/oauth/callback`). For Microsoft, the scope `Calendars.ReadWrite` is requested automatically. See [CALENDAR_INTEGRATION.md](CALENDAR_INTEGRATION.md) for details.
...
- **Admin verification**: Only the verified admin controls the bot; with Inbound switched on for Discord, anyone else who writes the bot a direct message is answered as a contact of the admin's book (see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md))
- **Persistent bridge**: Messages are routed through the headless agent; replies are sent back to Discord automatically
- **Message history**: The per-chat conversation is the source of truth; `read_discord_chat` reads it directly (always complete, including the agent's own replies), and `find_discord_messages` / `inbox` query a searchable index that is re-synced from the conversation on use.
...
The Discord window (Settings → Connections → Discord) uses the shared channel layout (`web/components/connections/ChannelDashboardShell.tsx`, the same shell as WhatsApp and Telegram): the chats on the left with a badge (Admin for the paired admin's direct message, Contact for a person an open Inbound answers, Read-only otherwise), the conversation in the middle, and a gear that opens the settings (Developer Portal link, bridge state with the start/stop switch, the admin, the recent activity). Strings live in `settings.discordDashboard` and `settings.channelDashboard`. `GET /api/discord/dashboard` carries `sessions`: the chats the message store holds under the admin, newest first, with `type` (`admin`, `contact` or `readonly`, the inbox's mode) and the inbox's `unread`, `waits`, `waits_reason`, `answered_by_agent` and `done` (see [INBOX.md](INBOX.md)); the window lists those rows (the paired admin's row from the activity log stands in until the store holds a message) with the same chips and the same "N waiting for you" button as the other channel windows.
...
- **Front Office on Telegram**: with Front Office switched on for Telegram (Settings → Connections → Front Office; `channel_ingress_policy.telegram.open_to_new_senders`) everybody the owner has not decided about is answered in Front Office mode and a new sender is added to the owner's contact book with no decision on them; a person the owner blocked is still turned away. Because the bot is shared by every account, the enrolment works only while exactly one owner is on the whitelist. With Front Office off, the default, only a contact the owner set to **allowed** is answered, and everybody else is turned away. The whitelist and the relay whitelist are the owner's own pairing and are not affected.
- The whitelist is the OWNER's pairing: a number on it gets the full agent. Everybody else is answered only through Front Office, which is the bullet above: a contact you allowed, or, while Telegram Inbound is on and exactly one owner is on the whitelist, somebody you have not decided about. Anyone else is not answered at all (no queue task, no agent run), which keeps the abuse and amplification risk down; the message itself is kept in the channel store so you can read it in the Telegram window and through the inbox tools.
- Every drop at ingress is recorded in the channel's own inbound log (`telegram_inbound_<date>.log`, `whatsapp_inbound_<date>.log`, `discord_inbound_<date>.log`: one `REJECT` line per sender and throttle window, written with debug logging off too). It is not a security event: a stranger writing to the bot is the channel's everyday traffic, and the security log is for what is unprotected or attacked (pairing changes are recorded there, see [SECURITY_DASHBOARD.md](../security/SECURITY_DASHBOARD.md)). "Silently" means the sender gets no reply, not that the attempt goes unrecorded.
- RAG, memories, and user identity are scoped per user, same as in the Web UI, and what the agent learns inside a contact's chat is scoped to that chat on top (see the 15-message rule below).
...
- **Dashboard:** The Telegram window uses the shared channel layout (`web/components/connections/ChannelDashboardShell.tsx`, the same shell as WhatsApp and Discord): chats on the left with a badge (Full access for a paired VAF user, Relay for a relay contact, Read-only otherwise), the conversation in the middle, and a gear that opens the settings (bot and bridge state, the paired users, the relay list with add/remove, the activity chart). Strings live in `settings.telegramDashboard` and `settings.channelDashboard`. The `sessions` of `GET /api/telegram/dashboard` are the paired and relay entries plus the chats the message store holds; the count, the newest message (`last_preview`, `preview_from`) and the person's own state (`unread`, `waits`, `waits_reason`, `answered_by_agent`, `done`) are the store's, read through the same overview the inbox reads (see [INBOX.md](INBOX.md)); the activity log only seeds a chat the store never saw and feeds the chart. The window shows that state on every row (the unread pill, the Waits for you chip, Agent answered, Done), offers the "N waiting for you" button in the list header, and marks a chat read when it is opened.
- **Conversation view:** Selecting a chat in the dashboard sidebar shows the conversation inline (user and bot bubbles, oldest first, auto-scrolled to the newest message; `<think>` blocks are stripped since they are never sent to the channel). The search over chats sits at the top of the chat list (it matches names, previews and ids). A search field on the conversation header works like Ctrl+F over the chat: matches are highlighted, Enter / Shift+Enter (or the arrow buttons) jump between them, Escape clears the search. The Memory Learning progress line is shown in the conversation footer for a chat whose session compacts: the Telegram route omits the counter for a relay contact (answered by nobody, never compacted), and the WhatsApp window shows it only for the owner's own chat.
- **After-15-messages rule:** The same **session compaction** as in the Web UI applies: every N **main-user** turns (default 15, configurable via `memory_compaction_interval`), the model is prompted to write durable memories into RAG. The prompt includes only **user and assistant messages** (no system or tool content). The count is **cumulative** (e.g. 4 today + 5 tomorrow = 9; at 15 total, compaction runs). Only role **user** (main user of that session) is counted; other participants (relay contacts, other bot users) have separate sessions. Memories are stored under the whitelist user's `user_scope_id`, so they appear in the same Memory graph and are used in later Web and Telegram chats. A **contact chat** the agent answers (Front Office contact, open conversation) compacts on the same interval into that chat's own memory namespace: the facts are about the contact and what was agreed with them, they appear in the Memory graph under one node per chat, they are read back only by turns inside that chat and by the WhatsApp Composer for that chat, and they are NOT used in the owner's Web or channel chats. Deleting the chat node on the Memory page removes them permanently. Relay contacts learn nowhere. Reply length: `memory_compaction_max_tokens` (default 4000). See [MEMORY_SYSTEM.md](../memory/MEMORY_SYSTEM.md#session-compaction-background).
...
- **Answer read-only chats yourself**: a chat the agent does not answer (a sender nobody allowed, or the whole channel with `inbound_to_agent` off) has a compose box under the conversation that sends from the agent's number, and the Composer beside it drafts into that box in the tone this number writes in, knowing what the agent learned in that chat if it ever answered there. Your own message changes nothing for the agent: it answers where it is allowed to, and writing to somebody by hand is not a permission. Details in [WHATSAPP_INTEGRATION.md](WHATSAPP_INTEGRATION.md#dashboard).
...
- **Who is answered**: the registered main-user number (full chat), every contact you set to **allowed** (on every channel where you have their address, whatever the switch says), and, while Front Office is switched on for WhatsApp in Connections (`channel_ingress_policy.whatsapp.open_to_new_senders`, off by default), everybody you have not decided about, who is then added to the contact book. A contact you **blocked** is answered nowhere. See [WHATSAPP_INTEGRATION.md](WHATSAPP_INTEGRATION.md#roles-the-linked-account-is-the-agent).
- **Read-only for everyone else**: Messages from other numbers are stored for you (the dashboard lists the chat as read-only, the inbox tools read it) and recorded in the channel's inbound log; the agent never answers them. The same rule holds for Telegram (any sender) and Discord (DMs; guild chatter is not kept): the ingress policy decides who is answered, not what you may read on your own bot.
...
- **Agent tools**: `inbox` (channel `whatsapp`), `find_whatsapp_messages`, `read_whatsapp_chat` list/search/read chats. **`send_whatsapp`** sends text, voice messages, or **documents (PDF, etc.)** to the user – WhatsApp as a channel where the bot can send the user content. `whatsapp_call` is a placeholder (not implemented).
...
#### Agent WhatsApp tools (inbox, find_whatsapp_messages, read_whatsapp_chat, send_whatsapp)

| Tool | Purpose | When to use |
|------|---------|-------------|
| `inbox` | Every conversation across every channel, newest first, with unread and who waits; `channel="whatsapp"` narrows it (see [INBOX.md](INBOX.md)). Returns chat_id, name, last activity. Use `find_whatsapp_messages` to search; `read_whatsapp_chat` to read a chat. | User asks "list WhatsApp chats" or "show my WhatsApp conversations". |
...
| `read_whatsapp_chat` | Read messages from a chat (`chat_id`, `limit`). Use chat_id from the inbox or find_whatsapp_messages. | read_whatsapp_chat(chat_id="+49...") for full thread. |
...
The WhatsApp window (Settings → Connections → WhatsApp) is laid out like the mail client: chats on the left with a badge saying who each one is to the agent (Owner, Contact, Assign number, Read-only, the lane's own answer), the active conversation in the middle, and a gear that opens the settings (agent number, your own number, who else may write, reply window, activity). Details in [WHATSAPP_INTEGRATION.md](WHATSAPP_INTEGRATION.md#dashboard).
...
- **A reply from someone the agent wrote to is not answered**: that is the rule, not a fault. Writing to somebody never opened a door, and the 72 hour reply window that used to open one is gone (it answered people on channels the owner had closed). Set the person to **allowed** in the contact book, or switch Inbound on for that channel.
- **Contact with Front Office doesn't get a reply**:
  - **Inbound off is not the whole answer**: a contact set to **allowed** is answered with Inbound off; a contact set to **blocked** is answered with Inbound on. Only people you have not decided about depend on the switch, and the hint under the control in the contact book says which case you are looking at.
  - The contact must have their **WhatsApp number** in the contact's **Channels** (phone/WhatsApp, E.164 e.g. +491701234567). Add it in Settings → Connections → Contacts → select the contact → **Add channel** in the Channels card (opens the edit form with the channels editor).
  - **Diagnose:** Check `logs/whatsapp_qr.log` for `[inbound] MESSAGE`, `[inbound] REJECT` (with `reason=` and `access=`, the latter being `allowed`, `denied` or `undecided`), or `[inbound] ACCEPT` / `[inbound] ENQUEUED`. Python also logs received event types (`[Python] got type='message'`) and JSON decode or read-loop errors in the same file. If you see "voice downloaded" but no `[inbound]` or `[Python] got type='message'` lines, restart the **full VAF application** (not only the bridge), then try again. See [WHATSAPP_INTEGRATION.md](WHATSAPP_INTEGRATION.md#diagnostic-logs-logswhatsapp_qrlog) for a full list of diagnostic lines.
...
Contacts provide a **central list** of people with **channel IDs** (WhatsApp, Telegram, Discord, email), a **personal file** per contact (language, how to address, birthday, notes, company, role, tags, status) and, in the Contacts window, one **timeline** of everything the stores hold about the person (messages, mails, notes, events). In your own chat the agent can resolve a name (e.g. “Max”) to that contact’s channels and then use the usual read/find tools (e.g. “Has Max written to me?” → `get_contact(name="Max")` then `read_whatsapp_chat` or `find_mail`); a Front Office turn has none of these tools, only `contact_history`, which reads the one person it is answering across every channel (see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md#tool-restriction)): the contact is the hub, so Bob asking on WhatsApp about the mail he sent is answered from Bob's own mail. Once Inbound (the Front Office) is switched on for a channel (Settings → Connections → Inbound, the card under Contacts), everybody you have not decided about is answered there by your assistant (on mail: everyone your mail provider verified as the sender, and by default as a draft you approve first, see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md#mail)), and **Let the agent reply** is the per-person control that overrules the switch in both directions; the assistant handles them in your context (like a front office for you), not as a separate user account.
...
- **The sync never decides anything about a person.** Whether the agent may answer for somebody is the user's own answer in the contact book, not something a chat list decides, so a synced record carries no decision at all. That is also why a record without one means "nobody decided" and never "blocked": reading it as a refusal would silence every person the channel sync had ever named.

The link is stored as `links.whatsapp = {endpoint, display_name, last_seen_ts, linked_at}`. The Contacts window shows the channel icons in the list row together with the relative time of the newest `last_seen_ts` over every link (the default sort order of the list), "Last contact" in the Key figures card, the WhatsApp messages themselves in the activity timeline, and a "Contact added, imported from WhatsApp" entry at the bottom of that timeline when the sync created the record (`linked_at`, see the **Added** field below). `get_contact` reports the link to the agent as `Linked via whatsapp, last contact <date>` plus `(shown there as <name>)` when the WhatsApp name differs from the contact's. Further channels hang their own key under `links` the same way, but only a real one-to-one conversation may create a contact: mail and Telegram are deliberately not synced automatically, because a mailbox or a bot list would import newsletters, notification senders and bots as people. The one exception is the same on every channel: a sender the open Front Office answers is enrolled as a contact with NO decision on the record (on mail only a person the provider verified and the machine-mail rule let through, never a newsletter). The open channel is what answers them, so closing it closes it for them again, and the record is where the owner allows or blocks them for good.

### Personal file fields

All optional. Every label in the Contacts window follows the UI language (`settings.contactsDashboard` in the message catalogues; German is the master).
...
| **Company** | `company`: the company or organisation the person belongs to; shown under the name in the detail header (`company · role`) and in the list row. |
| **Role** | `role`: the person's role or job title. |
| **Tags** | `tags`: a list of free labels. Whatever arrives (a list from the window and the routes, a comma-separated string from the agent tools) is normalised by `contacts_store._normalize_tags` the same way: whitespace collapsed, empty entries dropped, duplicates removed case-insensitively keeping the first spelling, at most 40 characters per tag and 20 tags per contact; `null` or anything that is not a list or string stores an empty list, never `None`, so every reader may iterate the field. The search box matches tags, `list_contacts(tag=...)` filters by one; every tag in use is offered as a suggestion, most frequent first (`GET /api/contacts/tags/values`). |
| **Source** | `source`: who created the record: `manual` from the window, `agent` from `create_contact`, or the channel name when a sync created it (`whatsapp`). |
| **Added** | `created_at`: when the record was created. Records from before these two fields existed carry neither; for them `contacts_store.contact_created` takes the date and the source from the OLDEST channel link (`links[<channel>].linked_at`, that channel as source), so every contact in the book shows an "added" date: in the window's File card ("Source: WhatsApp, since <date>"), in the `Added: YYYY-MM-DD (<source>)` line of `get_contact`, as the `created` field of the overview and as the "Contact added" entry that closes the timeline. |
| **Let the agent reply** (`assistant_access`) | One switch, showing whether the agent answers this person right now. **On** means either you allowed them, and then the agent answers on every channel where you have their address even with that channel's Inbound off, or nobody decided and that channel's Inbound stands open, which the line under the switch says. **Off** means either you blocked them, and then the agent answers them nowhere even with Inbound on, or nobody decided and their channels are shut. Switching it off writes a refusal, so it holds when a channel opens later; a new or synced record starts with no decision and follows its channel until you touch the switch. The assistant processes them in your context (like a front office for you), not as a separate user. |
...
| **Events** | The contact's appointments are events of the user's calendar linked to the contact (`contact_ids`), added from the dashboard (with a reminder) or `update_contact(add_event_title=..., add_event_when=...)`; times are read in the user's timezone (`vaf.core.user_time`). `contact_events` reads them back in the record's old shape; a record's legacy `events` list is moved into the calendar the first time the scope's calendar is opened. Upcoming calendar events that mention the contact's name or address without being linked are shown next to them, from the same store. See [CALENDAR_INTEGRATION.md](CALENDAR_INTEGRATION.md). |

**Record-internal versus cross-store.** Status, tags, company, role and notes live inside the contact record, so they are isolated exactly like the record: one `contacts.json` per username or scope (see [USER_ISOLATION.md](../security/USER_ISOLATION.md)), and none of the record-internal functions (`contact_summary`, `contact_self_view`, the notes helpers) reads another file; the appointments are events of the user's calendar linked to the contact, read through `contact_events`, which opens nothing when the scope has no calendar. The Contacts window additionally shows **cross-store glances**: live reads of the SAME user's other stores, keyed by the same username and scope that picks the contacts file, never stored and best-effort by design (each source sits in its own try/except, a missing store answers nothing and is never created, so opening a contact materialises no database):

- `GET /api/contacts/{id}/timeline?limit=50&cursor=...&kinds=message,mail,note,event,created` (`contacts_store.contact_timeline`): one newest-first list of the stored messages per channel endpoint (WhatsApp keyed by `+<digits>` plus any `@lid` jid the persisted `lid_to_e164` map resolves to the same number, Telegram by numeric chat id, Discord by user id; deleted tombstones skipped), the mails per address, the record's dated notes, the contact's appointments from the calendar (placed at the moment they were created, `ref.when_ts` is the appointment) and its creation. Each item carries `kind`, `ts`, `channel`, `direction` (`in`/`out`), `title`, `body`, `source` and a `ref` (`chat_id` for a message, so the window can jump into the chat; `note_id`; `event_id`; the mail keys). `kinds` narrows the sources (the window's tabs), so a notes-only view opens no message store. Paging is by an opaque `cursor`, the sort key `(ts, kind, id)` of the last item shown: every source is asked for `limit + 1` rows at or before that timestamp (inclusive, so a second shared by several items is never skipped), the merge drops what sorts at or above the cursor, and `next_cursor` is set while more remains. `limit` is clamped to 1..200. A lane that takes longer than ten seconds answers an empty page marked `timed_out: true` instead of blocking the window.
- `GET /api/contacts/{id}/overview` (`contact_summary` over `contact_events` plus `contact_calendar_events`: status, last contact over every channel link, the contact's appointments as `events`, `upcoming_events` and `next_event`, newest notes, and the calendar events that mention the contact without being linked to it) also carries `stats` (`contact_activity_stats`: message count, how many left the agent's side, the oldest STORED and the newest message, per channel; the oldest stored message is not the first contact ever, because WhatsApp history loads on demand), `endpoints` (`contact_endpoints`: the store keys per channel, the value the window hands to a chat window) and `created` (`contact_created`, see the **Added** field).
- Two named boundaries of the glances: the mail lane reads the v2 store (`mail.db`: the address as a complete mailbox in From, To or Cc, parsed from the headers with `vaf/mail/addressing.header_addresses`, so `ann@example.com` never matches `joann@example.com`; Junk, Trash and Drafts left out; direction from the headers and the Sent folder) and, for a legacy per-username user (username set, no scope), only the legacy `email_sync.db`, which is INBOX-only and therefore inbound-only (`vaf/mail/tool_bridge.messages_for_address_merged`); and Discord rows are written under the literal admin identity, so the Discord lane runs for the local admin only. A v2 store that cannot be read is logged and the legacy rows still answer, the same degradation the mailbox listing has. A message item's `id` is `<channel>:<store key>:<message_id>`, so two messages in the same second are two items and two page keys.

Notes and events are added and removed through `POST/DELETE /api/contacts/{id}/notes[/{note_id}]` and `.../events[/{event_id}]`; status, tags, company and role through `PATCH /api/contacts/{id}`. Several contacts at once: `POST /api/contacts/bulk` with `{ids, status?, add_tags?, remove_tags?}` answers `{"updated": n}` (a `status` left out is unchanged, `null` or `""` clears it), `POST /api/contacts/bulk/delete` with `{ids}` answers `{"deleted": n}`; both are POST so nothing collides with `PATCH /{contact_id}`, and ids that are not in the caller's own file are ignored. Suggestions come from `GET /api/contacts/statuses/values` (the four defaults plus every status in use) and `GET /api/contacts/tags/values` (every tag in use, most frequent first), both as `{"values": [...]}`. Every route takes its identity from the authenticated caller.

### Where to manage contacts

**Settings → Connections → Contacts** (gear icon) opens the Contacts window (`web/components/connections/ContactsDashboard.tsx`), a wide two-pane window (`max-w-[95vw] h-[90vh]`; on a phone the list sits above the detail, see [MOBILE_UI.md](../web-ui/MOBILE_UI.md)):

- **List column**: a search over name, company, role, tags, every channel value and the names the channels show; status chips with counts (**All**, one chip per status in use, **No status**); a sort toggle (last contact over every channel link by default, or name); rows with an initials avatar, name, a green dot for a contact who can reach the agent, the status pill, the company or the "last contact via <channel>" line, the relative time and the channel icons. A checkbox appears on hover; any selection shows the **bulk bar** (`{n} selected`, a status select that also offers **No status**, a tag input with suggestions, **Apply**, **Delete selected** behind a confirmation dialog, deselect) which calls `POST /api/contacts/bulk` and `POST /api/contacts/bulk/delete`.
- **Detail header**: avatar, name, `company · role`, the status (click to type, suggestions from the known values), the tag chips (remove, **Add tag** inline), the actions **Open WhatsApp chat** / **Open Telegram chat** (shown when the overview's `endpoints` carry a key for that channel: the Contacts window closes and the channel dashboard opens on that chat; when the dashboard has no stored chat under that key it selects its first chat and says "No chat is stored here for this contact"), **Note** (focuses the composer), **Event** (opens the inline event form), a menu with **Edit** and **Delete contact**, and the **Let the agent reply** switch with a one-line hint naming what made it so: you allowed this person, you blocked them, or nobody decided and their channel's Inbound is open or shut. The switch shows the effect rather than the word on the record, so the server folds the two (`contacts_store.assistant_reach`) and the row carries `assistant_answers` with its reason; the browser never folds it, because it only has the policy flag and the flag is not the whole door. Switching it on asks once, in the same dialog the WhatsApp dashboard uses, whether the agent may answer this person in Front Office mode; switching it off asks nothing and writes a refusal.
- **Activity** card: tabs **All**, **Messages**, **Notes**, **Events**, **Emails** with counts (messages from `stats`, notes and events from the record, mails as loaded), a note composer (Enter saves), and the timeline grouped by day (Today, Yesterday, then the date) with an icon per kind: message bubbles (green when sent from the agent's side) with **Open in chat**, mails with subject and snippet, notes with author and a remove button behind a confirmation, "Event added" entries, and the "Contact added" entry at the very end (imported from a channel, or by you or the agent). **Load older entries** at the bottom follows `next_cursor`. A tab change reloads the timeline with `kinds=`; a failed or timed-out load says so instead of showing an empty list.
- **Right cards**: **Channels** (icon, label, value, copy to clipboard; **Add channel** opens the edit form), **File** (company, role, language, how to address, birthday with "in N days", the source with "since <date>", the free-form notes; **Edit** opens the form), **Upcoming** (own events with a remove button, calendar events that mention the person marked "From your calendar", an inline add form, past events folded away) and **Key figures** (messages, of which from the agent, oldest stored message, last contact; a hyphen where there is no figure).
- **Edit form** (**Add contact** in the header, **Edit** in the menu or a card): name, the channels editor (WhatsApp, phone, Telegram, email, Discord; a phone number counts as WhatsApp once the contact may write to the agent), company, role, tags as a comma-separated field with suggestions and the personal file fields. Whether the person may reach the agent is switched only in the detail header, behind its confirmation; a new record starts closed. A create or update that fails is reported in the form's footer next to the save button, and the form stays open with the typed values.
- Escape steps back one level through the shared registry (see [WEB_UI.md](../web-ui/WEB_UI.md), Escape): a confirmation dialog, then the edit form, then an open inline editor or menu, then the window.
...
answers and a timer the person scheduled (which fires later under the source of the chat it
belongs to) all send as before. The decision lives in
[`vaf/core/outbound_hold.py`](../../vaf/core/outbound_hold.py), the switch is `outward_send_hold`, and the parked call is stored as a CALL rather than as an outbound
message row, because an outbound row is a message that left, and nothing has left yet.

When somebody the agent may answer sends a message (via WhatsApp, Telegram, a Discord direct message or, verified by the mail provider, by mail) - a contact you set to **allowed**, or anybody you have not decided about while that channel's Inbound is on - the agent responds in **Front Office** mode: it uses a dedicated system prompt (role + anti-prompt-injection) and a restricted tool set so the contact cannot abuse the system. **The bot replies directly** to the contact on WhatsApp, Telegram and Discord, and on mail as a draft you approve first unless you switch the mail channel to send at once; the config key `front_office_contact_reply_require_approval` exists but is not read by any code at the moment (see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md), Reply approval). See [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md) for when it activates, which tools are allowed, and how to override the anti-injection text.

**Settings → Connections → Inbound** (the Front Office in the UI). The card right under Contacts (admin only, because the policy behind its switch is an instance-wide key) shows whether Inbound is on and how many contacts may reach the agent on every channel; the gear opens the Inbound window. The window owns a switch **per channel** (WhatsApp, Telegram, Discord, E-Mail): on means everybody you have not decided about is answered there in Front Office mode, and a new sender is added to the book with no decision on them. The switch grants nobody and writes nothing into the contact book, so switching it off leaves no permissions behind; one person is kept out for good by switching **Let the agent reply** off for them in the channel window or the contact book, and one person is answered whatever the channel says by switching it on. Switching on asks once, the way allowing a single contact does; a note says when a channel is not connected or WhatsApp forwards nothing to the agent. The window also holds the owner's **instructions** for those turns and the **knowledge**: documents learned into the Front Office lane of the memory store, which only a contact's turn reads, with the switch that lets those turns also read the owner's general memory (off by default). Routes and the framework half are described in [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md#the-front-office-window-in-connections); every change of the switch is a `front_office_changed` security event per channel (see [SECURITY_DASHBOARD.md](../security/SECURITY_DASHBOARD.md)).

### Agent tools and user identity

- Contact tools (`list_contacts`, `get_contact`, `create_contact`, `update_contact`, `delete_contact`) use the same user identity (username and user_scope_id) as the current Web UI or bridge session. Storage and lookup walk the caller's own scope and username paths, so contacts are found whether they were saved by scope (e.g. JWT user) or by username; the local admin's book (`data_dir/contacts.json`) is a candidate for the local admin only and never a fallback for another user, so a tenant with no or an empty contacts file reads an empty book, not the admin's. Scope/UUID format is normalized so different string representations of the same user resolve to the same contact list.
- **`list_contacts(status=..., tag=...)`**: Returns all contacts with name (status in brackets when set), **contact_id**, channels and `Tags: a, b` when set; `status` narrows the list to one label, `tag` to the contacts carrying that tag (case-insensitive). Use when the user asks who is in their contact list. The **contact_id** is required for `update_contact` and `delete_contact`.
- **`get_contact(name="...")`**: Returns one contact’s **contact_id**, channel IDs, personal file, company, role, tags, `Added: YYYY-MM-DD (<source>)`, status, last contact over any channel link, upcoming events, the newest notes, the channel links and, as `Recent activity (newest first)`, up to five of the newest stored messages and mails with this person (`contact_timeline(limit=5, kinds={message, mail})`, each as `YYYY-MM-DD HH:MM <channel> in|out: <text>`). If multiple contacts share the same name, the tool returns all of them with their contact_ids and instructs the agent to ask the user which one they mean before any update or delete.
- **`create_contact`**: Create a contact (required: name; optional: email, whatsapp_phone, telegram_username, preferred_language, how_to_address, birthday, notes, `assistant_access` (`allowed` or `denied`; left out, nobody has decided), company, role, tags as a comma-separated string). The record is stamped `source: agent`. Returns the new contact with contact_id.
- **`update_contact(contact_id, ...)`**: Update a contact by **contact_id** (from list_contacts or get_contact). Optional fields: name, email, whatsapp_phone, telegram_username, preferred_language, how_to_address, birthday, notes, `assistant_access` (`allowed`, `denied` or `undecided`), status, company, role, tags (a comma-separated string that REPLACES the current tags; an empty string clears them); `add_note` appends a dated note (author `agent`), `add_event_title` + `add_event_when` attach a dated event. `get_contact` reports status, last contact, upcoming events and the newest notes, so the agent finds everything about a person in one place.
...
- **Agent tools**: When at least one email account is connected, the agent can use `inbox`, `find_mail`, `read_mail`, `mark_mail_answered`, `label_mail`, and `send_mail`. Credentials are never passed to the agent; the transport layer resolves them by `account_id`. Access tokens are refreshed automatically when expired.
...
#### Agent email tools (inbox, find_mail, read_mail, mark_mail_answered, label_mail, send_mail)

| Tool | Purpose | When to use |
|------|---------|-------------|
| `inbox` | Every conversation across every channel; `channel="mail"` lists the mail threads of every connected account, newest first, with unread counts and whether the last word was the correspondent's. `account_id` and `folder` narrow it. Output: one line per thread plus an "IDs by index" block with `account_id`, `message_id`, `folder` for `read_mail`. Suspicious threads are hidden by the phishing filter. | User asks to check, read or show their mails or who is waiting. |
...
| `read_mail` | Return the full body of one message as plain text. Parameters: `account_id`, `message_id`, `folder` (default INBOX), optional `provider_message_id`. Use IDs from the inbox "IDs by index" block or from find_mail. | When the user wants to read a message; use the IDs from the inbox output (by index) or find_mail; do not ask the user for these. |
...
The `inbox` tool uses a compact list format (one line per conversation, a short date, the unread count and the waits-for-you marker) plus a separate "IDs by index" block for mail so more conversations fit in context and the agent can present N distinct entries when the user asks for "N mails". Best practice: when the user asks for a number, pass it as `max_chats`.
...
- **Agent filter scope**: `inbox` and `find_mail` apply a phishing heuristic filter before returning results to the agent. Messages over the configured risk threshold are hidden from tool results.
...
2. Choose a provider:
  ...
  - **Admin setup (once per instance)**: In the mail wizard, expand **For admins: OAuth client**. Enter Google and/or Microsoft Client ID and Client secret (from a Web application OAuth app), then Save. After that, everyone on this VAF instance gets one-click sign-in. Redirect URI depends on your mode: in TLS/network mode use `https://localhost:8443/api/email/oauth/callback` - `8443` is the effective proxy port on **any** platform (Linux/macOS/Windows) when the privileged port 443 cannot be bound by a non-root user; if the proxy does bind 443, use `https://localhost/api/email/oauth/callback`. In localhost mode use `http://127.0.0.1:8001/api/email/oauth/callback`. Tip: register **both** the `:8443` and the no-port (`:443`) variants so sign-in works regardless of which port the proxy binds. Do not commit the secret to source code. See “Gmail: Desktop vs Web OAuth client” below for creating the OAuth app.
   ...
   - **Other (IMAP/SMTP)**: Enter your email and password (or app password if 2FA). Optionally set IMAP/SMTP host/port; defaults are used for ~60 known domains (Gmail, Outlook, Yahoo, AOL, iCloud, GMX, WEB.DE, mail.com, T-Online, IONOS, 1&1, freenet, Posteo, mailbox.org, Zoho, Fastmail, Yandex, Mail.ru, Vodafone, Bluewin, A1, Orange, La Poste, Libero, Seznam). A refused login answers with what that provider actually needs.
...
- **Network-mode session binding**: In network mode, OAuth start requires an authenticated Web session, and a **LAN** callback actor must match the user encoded in OAuth `state` (username/scope); mismatches are rejected to prevent cross-user token attachment. A genuine **loopback** callback (the desktop flow - the provider redirects the system browser to `127.0.0.1`/`localhost`, which cannot carry the Qt window's session cookie) is instead trusted via the signed, single-use, PKCE-bound `state` alone. The loopback check uses the real client IP (the proxy overwrites `X-Forwarded-For` with the true peer), so a forged header cannot make a LAN request look local. This applies uniformly to Email, Cloud, and GitHub (shared guard).
...
- **Web application client** (recommended if you get “client_secret is missing”): In [Google Cloud Console → APIs & Services → Credentials](https://console.cloud.google.com/apis/credentials), create an OAuth 2.0 Client ID of type **Web application**. Add authorized redirect URI(s) that match your active mode exactly, for example `https://localhost:8443/api/email/oauth/callback` (TLS/network mode - `8443` is the cross-platform fallback when 443 is not bindable), `https://localhost/api/email/oauth/callback` (when the proxy binds 443), and `http://127.0.0.1:8001/api/email/oauth/callback` for localhost mode. Registering both the `:8443` and no-port variants covers every platform. Enable Gmail API for the project. Set **Client ID** and **Client secret** in VAF: env vars `VAF_EMAIL_OAUTH_GOOGLE_CLIENT_ID` and `VAF_EMAIL_OAUTH_GOOGLE_CLIENT_SECRET`, or Settings → Connections → Email OAuth. This is how many local/desktop apps that use a local callback server work (e.g. VS Code, Slack): they use a Web client and keep the secret only on the deployer’s machine.
- **Redirect URI**: Must match exactly (scheme, host, and port). In network mode with TLS, use the HTTPS callback URL of the proxy entry point; in localhost mode, use the HTTP backend callback URL.
...
- **Two-factor authentication**: IMAP has no second-factor step, so a provider with 2FA switched on cannot accept the mailbox password at all. It hands out an **app-specific password** instead, and that is what belongs in the password field. VAF says which one applies when a login is refused, and links the provider's own page for creating it.
...
- **GMX / WEB.DE / mail.com**: Two things, and both cause the same "authentication failed". Create an app-specific password (Account verwalten > Login & Sicherheit > Anwendungsspezifische Passwörter), and switch POP3/IMAP access on (Einstellungen > POP3/IMAP Abruf). The IMAP switch turns itself back off after a long idle period, so a working account can start failing with no change on your side.
- **T-Online**: uses a separate password for mail programs that exists without any 2FA. The customer-centre password does not work in a mail client.
- **Proton Mail / Tuta**: Proton serves IMAP only through its local Bridge application (enter `127.0.0.1` under advanced options and set `email_allow_private_hosts`); Tuta offers no IMAP at all.
...
- **TLS**: IMAP and SMTP use TLS; the server certificate is verified against the system trust store, and connections use a connect timeout (port 465 uses implicit SMTP_SSL).
- **Private / self-hosted mail servers (SSRF guard)**: By default VAF refuses IMAP/SMTP hosts that resolve to a loopback, private (RFC-1918), or link-local address, so a user cannot point the mailer at internal services (e.g. the cloud metadata endpoint). If you genuinely run a mail server on your LAN or the same host, set `email_allow_private_hosts` to `true` (top-level in `config.json`). Multicast/reserved/link-local addresses (including `169.254.169.254`) are never allowed, even with this flag.
- **Defaults**: Known domains get default IMAP/SMTP host/port; you can override in the advanced options. The list lives in `MAIL_PROVIDERS` (`vaf/core/email_accounts.py`), which carries the server settings and the auth guidance in one entry per provider. An address whose domain is unknown and whose host was left blank is refused with that instruction rather than probed against someone else's server.
...
- **Per-account credentials** (OAuth access/refresh tokens, IMAP/SMTP passwords) are **never** stored in `config.json`. They are stored in:
  ...
  - **Fallback**: If keyring is unavailable, an AES-256-GCM encrypted file under the platform data directory (see `Platform.data_dir()`) is used. Encryption is envelope-based: a random data key encrypts the file and is itself wrapped by a key-encryption key. That KEK is derived from a master passphrase (`VAF_MASTER_PASSPHRASE`, via scrypt) when one is set - in which case no encryption secret is written to disk - or otherwise a random key kept outside `config.json`: by default an owner-only file `secure_store.kek` in `~/.vaf`, or the OS keyring when `secure_store_kek_backend` is set to `keyring` (opt-in, because a VAF started by a supervisor script has no session bus to reach the keyring through). The encrypted file, its wrapped-key sidecar, the KEK file, and `config.json` are all written with owner-only (`0600`) permissions. Read-modify-write is serialized with a process-local and a cross-process lock, so concurrent writers (e.g. backend and CLI) cannot lose updates. Paths and behaviour are cross-platform.
