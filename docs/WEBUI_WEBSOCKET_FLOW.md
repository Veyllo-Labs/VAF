...
- **MCP servers:** `mcp_servers` (`servers: Array<{ name, command, transport, url, enabled, permission_level, connected, tool_count, error }>`) in response to `get_mcp_servers`; `mcp_server_saved` / `mcp_server_deleted` (`name`) on success (the frontend re-fetches `get_mcp_servers` to refresh status); `mcp_server_error` (`error`) on failure.
