...
**Tool-loop protection:** To avoid unbounded tool-call loops (e.g. the model calling the same tool with the same arguments repeatedly), the agent enforces a maximum of **15 tool-result cycles** per user turn. Redundant calls (same tool and arguments as an already executed call) are blocked and contribute to the retry/stop logic. In thinking mode, a `thinking_done` tool call causes an immediate exit from the tool loop with no further API requests. See **Thinking-Mode.md** → [Loop protection (API cost safety)](Thinking-Mode.md#loop-protection-api-cost-safety). All such stops are logged with `[LOOP_PROTECTION]` in `logs/backend_YYYY-MM-DD.log`.
