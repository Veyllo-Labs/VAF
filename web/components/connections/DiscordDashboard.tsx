...
export default function DiscordDashboard({ isOpen, onClose, config, onConfigChange, initialChatId }: DiscordDashboardProps) {
    ...
    const chats: ShellChat[] = useMemo(() => {
        ...
        // The paired admin's chat is the admin's; a person an open Inbound answers is a
        // contact (the server's `type`, the inbox's mode); any other row the store holds is
        // read-only.
        const rows: ShellChat[] = (data.sessions || []).map(s => {
            const isAdmin = s.chat_id === adminId;
            const isContact = !isAdmin && s.type === 'contact';
            return {
                id: s.chat_id,
                ...
                ts: s.last_ts,
                badge: isAdmin ? { label: t('badgeAdmin'), cls: BADGE_CLS.owner }
                    : isContact ? { label: t('badgeContact'), cls: BADGE_CLS.contact }
                        : { label: t('badgeReadOnly'), cls: BADGE_CLS.readOnly },
                subline: `${s.chat_id} · ${isAdmin ? t('subAdmin') : isContact ? t('subContact') : t('subReadOnly')}`,
                footer: isAdmin ? t('footAdmin') : isContact ? t('footContact') : t('footReadOnly'),
                unread: s.unread,
                waits: s.waits,
