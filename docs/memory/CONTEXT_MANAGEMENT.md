...
**Measured, and partially collected since:** the `memory_rag_k` clamp now exists ONCE, in `turn_memory_context` (`vaf/memory/rag.py`) - the four lanes that repeated it by hand (headless runner, automation, thinking mode, mail routes) call the primitive, and the room turn joined them as its first new consumer instead of becoming the fifth copy (`tests/test_a2a_room_learning.py` pins the absence). The wider seam is still open: the remaining sites hand-roll "retrieve snippets and hand them to a prompt" with their own queries (voice agent, the search tool, `rag.py` itself, `context_tools`), and the headless runner splices seven further ad-hoc context blocks inline. Cross Chat Hint adds no copy of any of them.
