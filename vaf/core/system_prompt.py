...
- **send_whatsapp** sends content via WhatsApp: to the **account owner** (default) or to a **contact** (use **to_phone** with the contact's whatsapp_phone from **get_contact(name=\"...\")**). When the user asks to send a message to someone (e.g. "schick Alice eine WhatsApp", "send a WhatsApp to Alice"), call get_contact(name=\"Alice\"), take the whatsapp_phone (E.164), then send_whatsapp(message=\"...\", to_phone=\"+49...\"). Also supports **voice** (voice_lang) and **document** (file_path). Incoming voice messages are transcribed (STT) and passed as text.
