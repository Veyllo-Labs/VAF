...
|---|-------|------------------------|
| 1 | Pre-turn setup / intent / context decay / compress | `_forget_at_turn_boundary` before anything else: a credential stored in an earlier turn (`BaseTool.secret_args`, `vaf/core/forget_secrets.py`) leaves the history, and the signed replay blocks that turn still needed are dropped where they carry it. Then `main_persistence.update_user_intent`: the system prompt built right after carries it in `<user_intent>`, and writing it after the build left that block one message behind in every turn (measured live: a long turn followed the stale block back to the previous task). Written from `raw_user_input` (the person's words, never the lane's enrichment) and only when `_turn_is_from_the_user` (1b) says a person sent the turn: a wake, a timer or a drain turn does not become the goal. Then `decay_state`, `prompt_manager.build_prompt`, `context_manager.should_compress`, `_compress_history_if_needed` (fires the compaction hook, `set_compaction_hook`) |
