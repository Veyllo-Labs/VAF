...
| `reask_pending` | Voice reflex in-call re-ask line: spoken when the agent re-asks its OWN question because the owner asked to repeat it (the pending-answer path in `voice_turn.VoiceTurnEngine.turn`, `voice_policy.answer_verdict`); ~35 langs | `{question}` |
| `confirm_yes` / `confirm_no` | Matched lexicons (not spoken): the affirmations and negations `speaker_confirm.parse_reply` accepts as an answer to "did you mean me?" / a confirmation card. LEADING match only, and NO is checked before YES because several languages embed an affirmation inside a negation; the built-in pattern stays as the fallback for elongations ("jaaa") and if the book cannot load | none (word/phrase lists) |
| `capability_denial` | Matched lexicon (not spoken): phrases in which a reply claims the agent CANNOT do something (`voice_agent.denies_capability`). Consumed by the belt that replaces such a denial with the delegate acknowledgement when a delegation survived every gate, so the spoken words follow the deed | none (phrase lists) |
