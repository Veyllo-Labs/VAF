...
- **Admin Access:** Users with the `admin` role (including the local admin) have an **aggregated view**: they see their own scoped tasks AND legacy "root" tasks stored directly in `automations/` (without a subdirectory). Regular users are strictly restricted to their own scoped directory.
