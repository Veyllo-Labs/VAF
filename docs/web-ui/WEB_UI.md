...
- **Reach**: the "Train tool now" confirmation, the WhatsApp window's "let the agent reply" switch, and the Contacts window (delete a contact, delete the selection, remove a note or an event, all `destructive`; enabling Front Office for a person, not). The rest of the app still confirms ad hoc through the browser's `window.confirm()` or a hand-rolled card; converting those is a separate sweep.
