...
- **Front Office on Telegram**: with Front Office switched on for Telegram (Settings → Connections → Front Office; `channel_ingress_policy.telegram.open_to_new_senders`) every sender is answered in Front Office mode and a new one is added to the owner's contact book, unless that person's "Can reach your assistant" is off; because the bot is shared by every account, this works only while exactly one owner is on the whitelist. With Front Office off, the default, a contact with the flag is answered only through the expert contact door (`allow_contact_fallback`), and everybody else is turned away. The whitelist and the relay whitelist are the owner's own pairing and are not affected.
...
- **Answer read-only chats yourself**: a chat the agent does not answer (a sender nobody allowed, or the whole channel with `inbound_to_agent` off) has a compose box under the conversation that sends from the agent's number, and the Composer beside it drafts into that box in the tone this number writes in, knowing what the agent learned in that chat if it ever answered there. Your own message opens no reply window for the agent. Details in [WHATSAPP_INTEGRATION.md](WHATSAPP_INTEGRATION.md#dashboard).
...
- **Who is answered**: the registered main-user number (full chat), everyone who writes while Front Office is switched on for WhatsApp in Connections (`channel_ingress_policy.whatsapp.open_to_new_senders`, off by default; a person with "Can reach your assistant" switched off is the exception, and a new sender is added to the contact book), contacts with "Can reach your assistant" through the expert contact door (`allow_contact_fallback`), and anyone your agent wrote to inside the reply window (`whatsapp_config.reply_window_hours`, default 72; Front Office, reason `open_conversation`). See [WHATSAPP_INTEGRATION.md](WHATSAPP_INTEGRATION.md#roles-the-linked-account-is-the-agent).
...
- **Contact with Front Office doesn't get a reply**:
  - **Inbound must be on for WhatsApp**: Settings → Connections → Inbound (the Front Office card under Contacts). With it off every contact is turned away, whatever the contact's own switch says; the hint under that switch in the contact book says so.
...
| **Can reach your assistant (front office)** | With Front Office switched on for the channel in Connections, this is the per-person switch: off keeps the contact out, on (set for every contact of the channel when the channel is switched on, and for a new sender when they write) lets them reach your assistant via their channel IDs (WhatsApp, Telegram, Discord). The assistant processes them in your context (like a front office for you), not as a separate user. |
...
When a contact with **Can reach your assistant** enabled sends a message (via WhatsApp, Telegram, a Discord direct message or, verified by the mail provider, by mail) and the Front Office door of that channel is open, the agent responds in **Front Office** mode: it uses a dedicated system prompt (role + anti-prompt-injection) and a restricted tool set so the contact cannot abuse the system. **The bot replies directly** to the contact on WhatsApp, Telegram and Discord, and on mail as a draft you approve first unless you switch the mail channel to send at once; the config key `front_office_contact_reply_require_approval` exists but is not read by any code at the moment (see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md), Reply approval). See [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md) for when it activates, which tools are allowed, and how to override the anti-injection text.

**Settings → Connections → Inbound** (the Front Office in the UI). The card right under Contacts (admin only, because the policy behind its switch is an instance-wide key) shows whether Inbound is on and how many contacts are allowed; the gear opens the Inbound window. The window owns a switch **per channel** (WhatsApp, Telegram, Discord, E-Mail): on means everyone who writes there is answered in Front Office mode, every contact of that channel in every contact book on this instance is allowed at that moment and a new sender is added to the book when they write; one person is kept out by switching **Can reach your assistant** off for them in the channel window or the contact book. Switching on asks once, the way allowing a single contact does; a note says when a channel is not connected or WhatsApp forwards nothing to the agent. The window also holds the owner's **instructions** for those turns and the **knowledge**: documents learned into the Front Office lane of the memory store, which only a contact's turn reads, with the switch that lets those turns also read the owner's general memory (off by default). Routes and the framework half are described in [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md#the-front-office-window-in-connections); every change of the switch is a `front_office_changed` security event per channel (see [SECURITY_DASHBOARD.md](../security/SECURITY_DASHBOARD.md)).
...
- **`create_contact`**: Create a contact (required: name; optional: email, whatsapp_phone, telegram_username, preferred_language, how_to_address, birthday, notes, allow_as_assistant_user, company, role, tags as a comma-separated string). The record is stamped `source: agent`. Returns the new contact with contact_id.
- **`update_contact(contact_id, ...)`**: Update a contact by **contact_id** (from list_contacts or get_contact). Optional fields: name, email, whatsapp_phone, telegram_username, preferred_language, how_to_address, birthday, notes, allow_as_assistant_user, status, company, role, tags (a comma-separated string that REPLACES the current tags; an empty string clears them); `add_note` appends a dated note (author `agent`), `add_event_title` + `add_event_when` attach a dated event. `get_contact` reports status, last contact, upcoming events and the newest notes, so the agent finds everything about a person in one place.
