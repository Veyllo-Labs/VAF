...
| a contact you set to **allowed** (**Let the agent reply**) | `contact_allowed` | Front Office (restricted tools, dedicated prompt), on every channel where you have their address and whatever the Inbound switch says; learns into the chat's own memory namespace |
| anybody you have **not decided about**, while Inbound is on for WhatsApp | `front_office_open` | Front Office; they are added to your contact book with no decision on them, and the prompt says there is no standing permission; learns into the chat's own memory namespace |
| a contact you set to **blocked** | `contact_denied` | not answered, on any channel and with Inbound on; the message is stored for you like any other |
...
- **Ingress control**: Only the registered main-user number, contacts you set to **allowed**, and, while Inbound is on for WhatsApp, everybody you have not decided about are answered (see Roles above). A 72 hour reply window used to answer anybody the agent had written to, on closed channels included; it is gone. The policy decides who the agent answers, not what you may read: every inbound message from a person is stored, a rejected sender's too, so the dashboard shows your chats the way a WhatsApp client does. Not stored: the linked account's own chat (dropped as self-chat before the store) and anything that is not a person (status, newsletters, broadcasts, groups), which never reaches the ingress decision
- **Agent Tools**: `inbox` (channel `whatsapp`), `find_whatsapp_messages`, `read_whatsapp_chat`, `send_whatsapp` for listing, searching, reading, and sending
- **WhatsApp Inbox (message store)**: Like mail/Telegram, every message that passes through the bridge is stored in a local SQLite DB (`channel_messages.db`, next to the person's `chat_marks`: seen, done, asked-by-the-agent, see the channel model in CONNECTIONS.md). The dashboard's chat list is built from the **bridge** list + **activity** (including rejected senders) + the registered owner numbers and the contacts you allowed (their chats appear before any message exists) + the **store** (chats that have at least one stored message); the Chat list bullet in the dashboard section has the details. So all chats with messages stay visible and searchable even after a reconnect or when WhatsApp sends only a subset of chats. The `inbox` tool and the inbox window list the store only: a chat is a row there once it holds a message (see [INBOX.md](INBOX.md)).
...
| `reply_window_hours` | number | DISPLAY ONLY: how long a conversation the agent itself started counts as live in the WhatsApp window. Default `72`; `0` hides the line. It admits nobody, and the reply lane does not read it: who is answered is the person's own state in the contact book plus the channel switch. Until this round the same number was a door, and one message from the agent handed its recipient three days of access on a channel the owner had closed. The value is measured from the newest outbound message the AGENT sent in the per-user message store (a message the person sent from the dashboard opens nothing), and a reply inside it extends it |
...
- **Contacts (Front Office)**: A contact you set to **allowed** under **Let the agent reply** can write to your assistant (handled in your context, restricted tools), whatever the Inbound switch says. For WhatsApp, the contact **must** have that WhatsApp number stored in their **Channels** (type "phone" or "WhatsApp").
- **The reply window is display only.** When the agent writes to a number (`send_whatsapp(to_phone=...)`, or a reply in a Front Office chat), the window's timestamp says how long that conversation counts as live: the WhatsApp window shows it, and nothing else reads it. It admits nobody. It used to: for `reply_window_hours` (default 72) anybody the agent had ever written to could write back and be answered, in every mode, so one `send_whatsapp` on a channel the owner had closed handed that person three days of access (live incident). Whether a reply is answered is the same question the first message asked: the owner's own number, the person's state in the contact book, or an open channel. The rule itself has one implementation left, `vaf/core/inbox.reply_window_until` over the store's overview: the agent's own message opens the window, a reply inside it extends it, a later message does not, and a message YOU send from the dashboard's compose box opens nothing at all (it is stored as an outbound row under `OWNER_SENDER`, which the rule leaves out). The bridge's own copy (`conversation_open_until`) is gone with the door it served. NAMED BOUNDARY: `reply_window_hours` is kept because the window still shows the value; deleting the setting outright touches 93 sites in 26 files and is its own round.

The bridge resolves the sender once: the config whitelist decides the owner's own number, and the contact book is read for THAT person's record (`contacts_store.find_contact_by_channel`, then `contact_access` for their state), from the user's OWN book only (their scope file, then their legacy per-username file). The phone-set detour is gone with the reply window: a set built from the old flag could say "allowed" while the record said "denied", and the set was consulted first, so the denial was never read. The local admin's book is never consulted for another user: it used to be, which let a person the admin had cleared write to any user's agent number. A reply-lane call that arrives without a scope resolves it from the account name (`scope_id_for_username`) instead of defaulting to the admin's. The reply lane asks the SAME function: the headless runner's reply and the owner delivery go out only to a registered number, a contact the owner allowed, or anybody while Inbound is open (`_is_reply_allowed`, which calls `evaluate_ingress`); an unresolved `@lid` matches none of them. Asking it twice in two shapes is how the two sides drift, and the drift is silent: the agent could answer somebody who is no longer allowed to write.
...
- **Human takeover on the agent's phone**: When a person answers a contact from the agent's phone itself (e.g. someone opens the chat with Bob on the company phone and types), the bridge records "owner has control" for that chat. The agent **pauses replying** in that chat for **10 minutes** after the last such message. Incoming messages from the contact are still stored (message store + activity) but not handed to the agent until 10 minutes have passed with no further human message. In logs you will see `owner_sent chat=… → owner has control` and `owner_control: skip reply (owner has control, 10 min not elapsed)`. **Echo handling:** Only real human messages (sent from the phone) set owner control. When the bridge sends a reply, WhatsApp echoes it back with `fromMe: true`; the Node tracks recent sends per chat (text and voice) and does **not** emit `owner_sent` for those echoes, so the agent can keep replying to the next contact message. The message itself is stored as your own row (`OWNER_SENDER`, with the event's `body`, `message_id`, `content_type` and `msg_ts`; a captionless picture or document arrives as its `<media:type>` placeholder, as the history does), under the same chat key the inbound path derives (the event carries `fromE164` when the LID resolved, so the row lands in the contact's chat and never in a phantom `@lid` row; a note to this number's own LID chat is dropped), so the inbox shows the reply and reads the chat as answered by you, never as "agent answered". A status post, a channel post or a broadcast-list send from the phone is dropped before the event. A reaction or a delete from the phone is not a reply and stores nothing. The bridge knows its own echoes by id as well as by body (Baileys hands the id back with the send, remembered whenever the send lands, and `send_result` carries it as `message_id`, a field Python does not read yet: the send-path row keeps its own key), so a chunked reply, a document with or without a caption or a voice note followed by text no longer slips through as yours. The history import labels an outbound row the bridge did not send the same way (the bridge's own sends carry Baileys' id shape, `3EB0` followed by hex, which tells them from the phone's sends, not from another desktop client linked to the same account), keeps what a row the store already holds says, and adds no second row for a bridge send the send path stored the moment it left (same chat, same text or the same media family, within ten minutes; a phone-shaped id is never deduped, so a reply from the phone that repeats the agent's words is kept). Inbound rows carry the message's own time (the payload's `ts`), so a backlog delivered after an outage keeps its order against a reply you sent from the phone meanwhile.
...
- **whatsapp_inbound.log**: each inbound message: `ACCEPT`, `REJECT from=<jid> reason=<reason> access=<allowed|denied|undecided>` (the reason is `not_paired` or `contact_denied`, with `unresolved @lid` appended when the sender is a LID no number is known for), `SELF_CHAT`, etc. The `REJECT` line is written even with debug logging off, one per sender and throttle window; the rest of the lane needs it on.
...
If a **known** Front Office contact (e.g. Bob) uses a chat that WhatsApp sends as **@lid** (e.g. `123456789012345@lid`) and the Node never sends `fromE164`, the bridge **rejects** their messages (`REJECT from=123456789012345@lid reason=not_paired ... unresolved @lid` in `whatsapp_inbound.log`). To allow that contact again, add a **manual LID→E.164 mapping** in config so the bridge can treat that LID as their phone number:
...
The mapping only tells the bridge which number that @lid represents; who is answered is then the ordinary question. That number must be on the whitelist (the owner's own), or belong to a contact set to **allowed**, or WhatsApp Inbound must be on, in which case a sender nobody has decided about is answered and enrolled as a contact like any other.
...
2. **Tool guard:** `send_whatsapp` reads `_agent._front_office_mode` in `run()`. While the agent handles a contact's message the tool sends only to the owner's registered number: a `to_phone` naming anyone else returns `[TOOL BLOCKED]` instead of sending, and a send without `to_phone` is the owner back-channel (`main_messenger = whatsapp`, FRONT_OFFICE.md) and goes through. An earlier version blocked the send without `to_phone` instead, which silenced that back-channel once the agent got its own number.
...
- **Python**: Drops on the Node-emitted `selfChat` flag only (never on `@lid` alone). For everyone else the resolved `fromE164` (or a manual `lid_to_e164` mapping) is what gets matched against the registered number and the contact book; an unresolved `@lid` matches nothing and is refused as `not_paired` before the policy is asked (it is not a denial: nobody decided anything about a sender the bridge cannot name).
4. **Activity**: Appends to `chat_activity` (for dashboard) and to the per-user message store (`direction="in"`).
5. **Enqueue**: Task is added with `session_id = whatsapp_{username}_{digits}`, `input_text = body`, and metadata: `from_contact` (set for every sender except the registered main-user number), `ingress_reason` (`explicit_pair` / `contact_allowed` / `front_office_open`), `whatsapp_chat_jid`, `voice_lang`, `user_scope_id`, `username`. When `inbound_to_agent` is `false`, this enqueue is skipped, and the Front Office enrolment is skipped with it (`messaging_connections.front_office_open`): a channel that hands nothing to the agent answers nobody, so nobody is admitted there and no `contact_access_changed` event is written. The message is still stored and still logged as accepted, and the older linking lane still records the sender in the contact book with `source` `whatsapp`.
...
| (get_contact_names from the dashboard) | `getContacts` | Node responds with `type: "contacts", contacts: [{jid, e164, name, source}]`: every named number in its contact store (`source` = addressbook / business / push). Python names the chats it lists from the message store with it and feeds the contact book (`sync_channel_contacts`), so names reach both even while the Node's own chat list is empty (it is, after a restart without a sync) |
| (resync_contacts from the dashboard) | `resyncContacts` (`req_id`) | Forgets the local version of the app-state contact collection (`critical_unblock_low`) and calls Baileys `resyncAppState` on it (through the socket's own key store, which is a cache over the files). Measured live: that collection's records carry only timestamps, no names, so this step alone yields nothing; the names arrive inside history batches. Python therefore follows up with one on-demand history request (`fetchHistory`, count 1) per stored chat, one per second, at most 120: WhatsApp answers each with that conversation, name included, and the Node folds `messaging-history.set` `contacts` and `chats` into its stores. Answers `type: "resync_contacts_result"` |
...
|------|---------|-------------|
| `inbox` | Every conversation across every channel (`channel="whatsapp"` narrows it): chat_id, name, last activity, unread, who waits | User asks to list or show WhatsApp conversations or who is waiting |
...
- **Names and pictures, as WhatsApp Web shows them**: the Node keeps a contact store from `contacts.upsert` (the phone's address-book names from the history sync and app-state `contactAction`, verified business names) and `contacts.update` / `msg.pushName` (the name a person set for themselves), keyed by JID with LID and phone JID pointing at one entry; the chat list resolves names through it, and every accepted inbound message is stored with that name (`chat_name`). The store (names, LID pairs and the chat list itself) is written to `contacts-cache.json` beside the credentials (2 s after the last change) and loaded on start, because WhatsApp sends the chat list with its names only with the first full sync after linking; a bridge that never saw that sync asks for it again with **Reload names from the phone** in the settings (`POST /api/whatsapp/contacts/resync`, the `resyncContacts` command). A chat with no name anywhere still shows the number, an unresolved LID "Unknown chat". Pictures come from `GET /api/whatsapp/avatar?chat_id=...` (see `getAvatar` in the command table); a person who hides their picture gets the initials.
- **Contact book**: every chat-list update (throttled to once a minute per user) and every accepted inbound message with a name is folded into the contact book: a named direct chat with a phone number is matched by number and linked, or created with its WhatsApp name; groups, newsletters and number-only chats are not; no decision about the person is ever written. Rules and the `links.whatsapp` field in [CONNECTIONS.md](CONNECTIONS.md#channel-links-whatsapp-today).
- **Jump from the contact book**: the Contacts window's **Open WhatsApp chat** action and the **Open in chat** link under a WhatsApp message in a contact's timeline close the Contacts window and open this dashboard on that chat (`initialChatId`, the contact's store key from `GET /api/contacts/{id}/overview` `endpoints.whatsapp[0]`, which equals the dashboard's `chat_id`). When the dashboard's chat list holds no chat under that id (a contact with a number but no stored messages), it selects its first chat and the conversation header says "No chat is stored here for this contact" (`jumpChatNotFound`); the note clears on the next chat selection. `SettingsModal` keeps the pending jump and clears it once both channel dashboards are closed. Telegram works the same way through its own dashboard.
...
- **Chat badges**: **Owner** = registered main-user number (full chat), **Contact** = a person the agent answers here (one you allowed, or anybody while Inbound is on), **Assign number** = a `@lid` chat WhatsApp has not resolved to a number yet (the conversation header offers an input that calls `POST /api/whatsapp/lid-assign`), **Read-only** = everything else, a person you blocked included (session `type` values `owner` / `contact` / `unknown` plus `needs_assign` in `GET /api/whatsapp/dashboard`). The badge is the lane's own answer: the route asks `vaf/core/inbox.chat_mode`, the same rule the bridge applies, instead of deriving a mode of its own. It used to hand-roll one, which is how a chat could carry a "Conversation, Front Office" badge while the bridge refused it.
- **Conversation**: the messages of the chat come from the per-user **message store** (`GET /api/whatsapp/chat-messages?chat_id=...`, oldest first; "in" = the other side, "out" = what left the agent's number), not from the agent session: a chat that arrived through the history sync has a full conversation in the store and no session until the agent itself answers there. The Memory Learning counter reads the session (`whatsapp_<user>_<digits>`) and travels only for the owner's own registered number while inbound messages reach the agent: a contact chat the agent answers learns too, into its own memory namespace, but its visible surface is the chat node on the Memory page rather than this counter; a read-only or manually written chat learns nowhere and shows none. **Load older messages** in the conversation header (`POST /api/whatsapp/chat-messages/older`, body `chat_id`, `count`) asks the phone for up to 50 messages older than the oldest stored one (`channel_message_store.oldest_message` is the cursor) and reloads the pane once they landed; WhatsApp hands over only what the phone still has, so "nothing older arrived" is a normal answer.
- **Unread, waits for you, done**: every row carries the inbox's state under its preview (the red unread pill, the amber **Waits for you** chip, **Agent answered**, **Done**), the list header's right side becomes an **N waiting for you** button that jumps to the next waiting chat, and opening a chat marks it read on every surface (see [INBOX.md](INBOX.md)).
- **Conversation header**: name, badge, number and a one-line explanation of what the agent does with this chat; **Add as contact** on any chat the book does not know yet creates the record and nothing more (`POST /api/contacts`): adding somebody is not the same as handing them a pass that outlives the Inbound switch. When the number is in the contact book, the header says so and shows the **Let the agent reply** switch, which is on exactly while the agent answers this person on WhatsApp: because you allowed them, or because nobody decided and Inbound is open here. Switching it on asks once whether the agent may talk to this person in Front Office mode; switching it off asks nothing and writes a refusal, which holds when Inbound opens again (`PATCH /api/contacts/{id}` with `assistant_access`; the dashboard payload carries `contact_id`, `contact_name`, `contact_access` and the folded `assistant_answers` / `assistant_reason` per chat). The reason rides along as the switch's title, because this header is a strip; the full sentence is in the contact book. **Load older messages** is a pill at the top of the message list, where scrolling up lands, not a header button. The search over chats and contacts sits at the top of the chat list; the search field in the conversation header searches inside the open chat. Where the agent answers, the footer names the mode (full agent, Front Office) and, for the owner's chat, the message count until the next Memory Learning; where the person writes themselves, the compose box takes the footer's place.
- **Compose box and Composer** (where the agent does NOT answer: a read-only chat, or every chat while `inbound_to_agent` is off; owner and contact chats are the agent's, and an unassigned `@lid` has no address): under the conversation a text field with **Send** (Enter sends, Shift+Enter breaks the line; the field grows with what you type to three lines and scrolls beyond that without a visible bar, the Composer's instruction field likewise) sends your own words from the agent's number, `POST /api/whatsapp/send` (`chat_id`, `text`); the bridge's sender loop stores the row and the pane reloads. The message is stored under `OWNER_SENDER`, so it is not the agent writing: the person's answer stays a read-only message for you, and writing by hand grants nobody anything. To the right of the conversation the **Composer** drafts into that text field, the same assistant as the mail window's on the shared lane (`vaf/core/composer.py` with the `CHAT` profile, `vaf/core/composer_lane.py`; design and containment in [EMAIL_CLIENT.md](EMAIL_CLIENT.md#mail-composer)): say what the reply should say or what to pay attention to, **Write a draft** reads the stored chat (newest inbound message first, your side labelled so the model matches how this number writes, memory keyed on your instruction, plus what the agent learned in this chat if it ever answered here), **Rewrite my text** works on what you typed and reads no chat messages (it still knows what the agent learned in this chat), follow-ups refine the last draft, **Stop**, **Undo**, and a result line that says how much of the chat was read and whether it had a sample of your tone (`POST /api/whatsapp/composer`, SSE frames identical to the mail route: `meta`, optional `notice`, cumulative text, `error` or `end`). One model call, no tools, nothing sent: you press Send. The panel is offered while `mail_composer_enabled` is on (`composer_enabled` in the dashboard payload); on a phone it stacks under the conversation. Switching chats keeps each chat's box and Composer exchange in the window's memory (a state swap, no request) and brings them back when you return; a draft or a send still running for the chat you left is bound to that chat, so its outcome lands in that chat's stash rather than in the one on screen; closing the window drops them all.
...
- **Chat list**: Built from (1) Node’s chat list (Baileys), (2) `chat_activity` (incoming/outgoing activity), (3) the contacts you allowed that have a WhatsApp channel, so those chats appear even before Baileys has synced them, and (4) the per-user message store, the persistent inbox: every chat it holds is a row. **Deduplication:** Each contact is shown once; the same person as E.164 and as @lid is merged into one row when LID→E.164 is known (config or Node). Phone numbers are normalized to a single leading `+` to avoid duplicate entries (e.g. `++49...`). The **message count** shown per chat is the **store's count** (the number the inbox and the agent's `inbox` tool show), and each row carries the person's own state from the same overview the inbox reads: `unread`, `waits`, `waits_reason`, `answered_by_agent`, `done`, `last_preview`, `preview_from` (see [INBOX.md](INBOX.md)). The badge is `inbox.chat_mode` over the same rows (the lane's own answer), and `reply_window_until` is `inbox.reply_window_until` over them; the dashboard asks the bridge for the chat list and the names only, never for a rule. **Contact names** are resolved from the contact list; matching uses canonical phone form (0-prefix German numbers, e.g. `0152...`, are treated as `+49...`) so names appear even if the contact was stored as `0152...` and the session uses `+49152...`.
...
3. **Verify sender is whitelisted:** Your phone number (E.164) must be in the config whitelist, or belong to a contact the agent may answer for (set to **allowed**, or, while Inbound is on for WhatsApp, a record nobody has decided about; a contact set to **blocked** is never answered, Inbound on or off), and that contact must have the WhatsApp number in Channels. Check `logs/whatsapp_inbound.log` for `ACCEPT` vs a `REJECT` line, whose `reason=` says which rule refused them and whose `access=` says what your book holds about them.
...
The contact must have their **WhatsApp number** stored in the contact’s **Channels** (type "phone" or "WhatsApp", value E.164). If the contact is set to **allowed** but has no WhatsApp channel, incoming messages are rejected. Add the number in Settings → Connections → Contacts → edit contact → Channels. **Diagnose:** Check `logs/whatsapp_qr.log` for `[inbound] MESSAGE`, `[inbound] REJECT` (with `reason=` and `access=`, the latter being `allowed`, `denied` or `undecided`), or `[inbound] ACCEPT`/`ENQUEUED`. Python also logs each received event as `[Python] got type='message'` and any `[Python] JSON decode error` or `[Python] FATAL read loop` in the same file. If you see "voice downloaded" but no `[inbound]` or `[Python] got type='message'` lines, restart the **full VAF application** (not only the bridge) so the bridge runs with UTF-8 encoding for Node pipes, then try again.
...
Only the numbers in the config whitelist, contacts you set to **allowed** who have a WhatsApp channel, and, while Inbound is on for WhatsApp, everybody you have not decided about receive replies. The linked account's own chat is dropped. All other senders are not answered and get no notification; their messages are stored for the owner to read. Chats identified by LID (`@lid`) are only treated as self-chat when the resolved E.164 matches the linked account owner’s number; other LID chats are subject to the same whitelist/contact checks as normal JIDs.
