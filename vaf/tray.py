...
# The stack lifecycle moved to the engine (vaf/core/service_stack.py): the
# tray carried the only complete implementation while `vaf run` started
# nothing, so a terminal-only start left the memory DB down and memory reads
# looked empty. Both launchers call the same primitive now; the tray keeps
# only its voice (the log prefix) and its call sites.
from vaf.core.service_stack import (
    ensure_service_stack,
    is_docker_daemon_running as _is_docker_daemon_running,
    stop_service_stack,
)


def ensure_memory_stack_up():
    """Start the Docker service stack (engine bootstrap + two-phase compose
    up). The whole behavior lives in vaf/core/service_stack.py now."""
    ensure_service_stack(log=lambda m: log("Tray", m))
