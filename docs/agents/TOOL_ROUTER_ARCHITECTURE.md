...
**Result truncation and the deliverable exemption:** the dispatch funnel caps every result (`max_result_chars`, 2000 in the chat lane) to protect the model's context from unbounded output. A tool whose result IS the deliverable declares `result_is_deliverable = True` on its class and is returned to the model whole, promising in exchange to keep its own output bounded (`use_skill`, `read_skill`, `room_invite` - all three measured in one live incident where the cap cut a skill body mid-instruction and an invitation briefing inside a block the result text orders the model to pass on "unchanged and complete" - and `read_file`, whose windowed reads the cap reduced to blind 2,000-char cuts with the line count and the start_line continuation facts destroyed; its `_cap_result` ceiling is the bounded-output promise, pinned by `tests/test_read_file_window.py`). The event stream stays capped independently (`event_result`), so observation never inherits the exemption.
