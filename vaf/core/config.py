...
class Config:
    ...
    DEFAULTS = {
        ...
        "workflow_identity_injection": "legacy",             # Who a workflow's tools think is calling. 'legacy' = the four consumers that never passed an identity keep passing none (a saved workflow runs as the machine owner whoever started it); 'declared' = they pass the real one. NOT a boolean: the three lanes that always passed an identity are unaffected either way
