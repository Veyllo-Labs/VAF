...
- **When:** Compaction runs **only for the main user's own sessions**: the Web UI chats and the main user's own Telegram, WhatsApp and Discord chats (a channel task without the `from_contact` flag). It does **not** run for contact chats on those channels, for data-protection reasons (DSGVO): the bridges flag every task from a contact (Front Office, open conversation, relay) with `from_contact`, and the headless runner skips compaction for it (`COMPACTION_SKIP reason=contact_chat_dsgvo`). After every chat task, the headless runner checks whether that session has reached the compaction interval (number of user turns since last compaction). Default: every **15 user turns** (`memory_compaction_interval`). The count is cumulative per session; only messages with role `user` are counted.
...
- **Contact chats (Telegram/WhatsApp/Discord):** Compaction is **disabled** for these sessions. Only the main user's own sessions (Web UI and the main user's own channel chats) are compacted into long-term memory. Contact conversations are never written to RAG (DSGVO-friendly), and messages the bridges only store without an agent run (rejected senders, a channel with inbound switched off, the owner's manual sends) never reach a session, so they cannot be learned from.
