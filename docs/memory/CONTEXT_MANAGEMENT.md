...
> **Archived chats reach BOTH memory lanes.** The hint lane below reads session files, so archives joined it by being in `iter_owned_sessions`. The RAG **snippet** lane is a vector store, and nothing writes into it except session compaction - which only ever ran on a LIVE history at a turn interval, so an archived chat would have reached the hints and never the snippets. `ingest_archived_chat` (`vaf/memory/rag.py`) runs the same compaction over the archived transcript: same prompt, same fact gates, same store, `source` set to `archive/<session id>` so `delete_memories_by_source_scope` can take back exactly what it taught, and a turn count that clears the interval gate because a finished chat gets one chance. It is queued as `__CMD__:ARCHIVE_INGEST` to the headless runner rather than called in the web request: compaction needs an Agent, and the runner is the process that owns one.
