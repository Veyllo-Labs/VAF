...
**Record-internal versus cross-store.** Status, tags, company, role, notes and events live inside the contact record, so they are isolated exactly like the record: one `contacts.json` per username or scope (see [USER_ISOLATION.md](../security/USER_ISOLATION.md)), and none of the record-internal functions (`contact_summary`, `contact_self_view`, the notes and events helpers) reads another file. The Contacts window additionally shows **cross-store glances**: live reads of the SAME user's other stores, keyed by the same username and scope that picks the contacts file, never stored and best-effort by design (each source sits in its own try/except, a missing store answers nothing and is never created, so opening a contact materialises no database):

- `GET /api/contacts/{id}/timeline?limit=50&cursor=...&kinds=message,mail,note,event,created` (`contacts_store.contact_timeline`): one newest-first list of the stored messages per channel endpoint (WhatsApp keyed by `+<digits>` plus any `@lid` jid the persisted `lid_to_e164` map resolves to the same number, Telegram by numeric chat id, Discord by user id; deleted tombstones skipped), the mails per address, the record's dated notes, its events (placed at the moment they were attached, `ref.when_ts` is the appointment) and its creation. Each item carries `kind`, `ts`, `channel`, `direction` (`in`/`out`), `title`, `body`, `source` and a `ref` (`chat_id` for a message, so the window can jump into the chat; `note_id`; `event_id`; the mail keys). `kinds` narrows the sources (the window's tabs), so a notes-only view opens no message store. Paging is by an opaque `cursor`, the sort key `(ts, kind, id)` of the last item shown: every source is asked for `limit + 1` rows at or before that timestamp (inclusive, so a second shared by several items is never skipped), the merge drops what sorts at or above the cursor, and `next_cursor` is set while more remains. `limit` is clamped to 1..200. A lane that takes longer than ten seconds answers an empty page marked `timed_out: true` instead of blocking the window.
- `GET /api/contacts/{id}/overview` (`contact_summary` plus `contact_calendar_events`: status, last contact over every channel link, next event, newest notes, matching calendar events) also carries `stats` (`contact_activity_stats`: message count, how many left the agent's side, the oldest STORED and the newest message, per channel; the oldest stored message is not the first contact ever, because WhatsApp history loads on demand), `endpoints` (`contact_endpoints`: the store keys per channel, the value the window hands to a chat window) and `created` (`contact_created`, see the **Added** field).
