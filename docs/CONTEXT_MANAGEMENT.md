...
Set `"tool_loop_unlimited": true` in config to disable the hard limit entirely (the soft reminder at 50 still fires). Redundant calls (same tool and same arguments as an already-executed call) are blocked independently of these counters. In thinking mode a `thinking_done` tool call exits the loop immediately. All limit events are logged in `logs/backend_YYYY-MM-DD.log` under `[LOOP_PROTECTION]` / `soft_limit_reminder` / `hard_stop_injection`. See also **Thinking-Mode.md** → [Loop protection (API cost safety)](Thinking-Mode.md#loop-protection-api-cost-safety).
