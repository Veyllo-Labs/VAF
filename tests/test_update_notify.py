...
def test_notify_disabled_does_not_fetch(monkeypatch):
    ...
    called = {"n": 0}
    monkeypatch.setattr(uc, "resolve_latest_release",
                        lambda: (called.__setitem__("n", called["n"] + 1), ({"version": "9.9.9"}, None))[1])
...
def test_fresh_cache_skips_fetch(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(uc, "update_cache_path", lambda: cache)
    called = {"n": 0}
    monkeypatch.setattr(uc, "resolve_latest_release",
                        lambda: (called.__setitem__("n", called["n"] + 1), (None, "offline"))[1])
    info = uc.cached_or_fetch_latest()
...
def test_stale_cache_fetches_and_recaches(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(uc, "update_cache_path", lambda: cache)
    monkeypatch.setattr(uc, "resolve_latest_release", lambda: ({"version": "999.0.0", "tag": "v999.0.0"}, None))
    info = uc.cached_or_fetch_latest()
...
def test_offline_returns_none(tmp_path, monkeypatch):
    monkeypatch.setattr(uc, "update_cache_path", lambda: tmp_path / "nope.json")
    monkeypatch.setattr(uc, "resolve_latest_release", lambda: (None, "offline"))
    assert uc.cached_or_fetch_latest() is None
