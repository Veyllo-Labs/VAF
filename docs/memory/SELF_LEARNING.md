...
**Effect over time:** More chats → more memories in RAG → better retrieval → more relevant context in each reply. Learning is **per user** (`user_scope_id`). The main user's own conversations (Web UI and their own channel chats) are compacted into the general lane; a chat the agent answers for a contact (Front Office, open conversation) is compacted into that chat's own memory namespace, which only turns inside that chat and the WhatsApp Composer for that chat can read, and which the owner deletes as one node on the Memory page (see [MEMORY_SYSTEM.md](MEMORY_SYSTEM.md#chat-memory-namespaces-what-a-contact-chat-teaches-stays-in-that-chat)).
