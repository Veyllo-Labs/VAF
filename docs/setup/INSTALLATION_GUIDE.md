...
- **"Hardware virtualization is DISABLED" / WSL error 0x80370102** — the installer checks this first (no admin needed for the check): Intel VT-x / AMD-V must be enabled in the BIOS/UEFI. Reboot into the firmware setup (Settings > Recovery > Advanced startup > UEFI Firmware Settings, or F2/F10/DEL at boot), enable "Intel Virtualization Technology" or "SVM Mode" (often under Advanced / CPU Configuration), save, and run the installer again. On the full Hyper-V role: not required — VAF only needs the hypervisor platform (WSL2), which also works on Windows Home.
