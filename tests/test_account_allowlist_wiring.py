...
def test_importing_vaf_main_registers_the_harness_resolver():
    ...
    script = (
        ...
        "    get_account_allowlist_resolver, get_agent_persona_resolver,\n"
        "    get_workflow_allowlist_resolver)\n"
        ...
        "assert get_agent_persona_resolver() is agent_display_name, (\n"
        "    'vaf.main did not register the persona resolver - every agent would '\n"
        "    'introduce itself to rooms as the product name')\n"
        "print('REGISTERED_OK')\n"
    )
    env = dict(os.environ)
