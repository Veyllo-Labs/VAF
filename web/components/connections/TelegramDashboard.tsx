...
export default function TelegramDashboard({ isOpen, onClose, config, onConfigChange }: TelegramDashboardProps) {
    ...
    const formatBucketLabel = (bucket_ts: number) => {
    ...
    const renderLineChart = () => {
        const buckets = data?.stats_4h ?? [];
        if (buckets.length === 0) return null;
        const maxCount = Math.max(1, ...buckets.map((b) => b.count));
        const width = 600;
        const height = 100;
        const padding = { top: 6, bottom: 22, left: 0, right: 0 };
        const w = width;
        const h = height - padding.top - padding.bottom;
        const points = buckets.map((b, i) => {
            const x = (i / (buckets.length - 1 || 1)) * w;
            const y = padding.top + h - (b.count / maxCount) * h;
            return { x, y, ...b };
        });
        const pathD = points.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`).join(' ');
        const lastX = points[points.length - 1]?.x ?? 0;
        return (
            <div className="rounded-lg border border-gray-200 bg-gray-50/50 overflow-hidden px-[30px] w-full min-w-0">
                <p className="text-sm font-medium text-gray-700 pt-3 pb-1">Messages per 4-hour interval (last 7 days)</p>
                <div className="w-full" style={{ aspectRatio: `${width} / ${height}` }}>
                    <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} className="block" preserveAspectRatio="xMidYMid meet">
                        <defs>
                            <linearGradient id="telegramChartGrad" x1="0" y1="1" x2="0" y2="0">
                                <stop offset="0%" stopColor="rgb(14, 165, 233)" stopOpacity="0.15" />
                                <stop offset="100%" stopColor="rgb(14, 165, 233)" stopOpacity="0" />
                            </linearGradient>
                        </defs>
                        <path d={`${pathD} L ${lastX} ${padding.top + h} L 0 ${padding.top + h} Z`} fill="url(#telegramChartGrad)" />
                        <path d={pathD} fill="none" stroke="rgb(14, 165, 233)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        {points.map((p, i) => (
                            <g key={i}>
                                <title>{p.count} message{p.count !== 1 ? 's' : ''} in this interval</title>
                                <circle cx={p.x} cy={p.y} r={p.count > 0 ? 8 : 0} fill="transparent" />
                                <circle cx={p.x} cy={p.y} r={p.count > 0 ? 3 : 0} fill="rgb(14, 165, 233)" />
                            </g>
                        ))}
                        <line x1={0} y1={padding.top + h} x2={width} y2={padding.top + h} stroke="#e5e7eb" strokeWidth="1" />
                    {[0, Math.floor(buckets.length / 2), buckets.length - 1].filter((i) => buckets[i]).map((i) => (
                        <text key={i} x={(i / (buckets.length - 1 || 1)) * w} y={height - 6} textAnchor="middle" className="fill-gray-500" style={{ fontSize: 5 }}>
                            {formatBucketLabel(buckets[i].bucket_ts)}
                        </text>
                    ))}
                    </svg>
                </div>
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            >
                                ...
                                <div className="rounded-lg border border-gray-200 bg-gray-50/50 max-h-40 overflow-y-auto">
                                ...
                                <div className="rounded-lg border border-gray-200 p-4">
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        ...
                                        <button
                                            ...
                                            className="px-3 py-1.5 rounded bg-gray-900 text-white text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            Add
                                        </button>
                                    </div>
