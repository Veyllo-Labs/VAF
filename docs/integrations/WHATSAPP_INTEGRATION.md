...
| (resync_contacts from the dashboard) | `resyncContacts` (`req_id`) | Forgets the local version of the app-state contact collection (`critical_unblock_low`) and calls Baileys `resyncAppState` on it, so WhatsApp streams the full snapshot again: every address-book name as `contacts.upsert`. For a bridge that missed the first sync (linked before the contact store existed, or the cache was lost). Answers `type: "resync_contacts_result"` |
...
- **Names and pictures, as WhatsApp Web shows them**: the Node keeps a contact store from `contacts.upsert` (the phone's address-book names from the history sync and app-state `contactAction`, verified business names) and `contacts.update` / `msg.pushName` (the name a person set for themselves), keyed by JID with LID and phone JID pointing at one entry; the chat list resolves names through it, and every accepted inbound message is stored with that name (`chat_name`). The store is written to `contacts-cache.json` beside the credentials (2 s after the last change) and loaded on start, because WhatsApp sends the address-book names only with the first full sync and app-state deltas afterwards; a bridge that never saw that sync asks for it again with **Reload names from the phone** in the settings (`POST /api/whatsapp/contacts/resync`, the `resyncContacts` command). A chat with no name anywhere still shows the number, an unresolved LID "Unknown chat". Pictures come from `GET /api/whatsapp/avatar?chat_id=...` (see `getAvatar` in the command table); a person who hides their picture gets the initials.
