...
| Messenger, inbox and mail window surfaces (`SFC_WINDOW` / `SFC_CHROME` / `SFC_CARD` / `SFC_CANVAS` / `SFC_RAISED` / `SFC_FILL` / `SFC_HOVER` / `SFC_ACTIVE` in `web/components/connections/ChannelDashboardShell.tsx`) | window `dark:bg-[#181818]`, chrome (header, nav, sticky bars) `dark:bg-[#1f1f1f]`, settings card `dark:bg-[#1c1c1c]`, the conversation floor `dark:bg-[#151515]`, a bubble or control sitting on that floor `dark:bg-[#262626]`, control fill on chrome `dark:bg-[#262626]`, selected row `dark:bg-[#2a2a2a]`. Eight pairs rather than eight folded neutrals, because for these the ramp INVERTS: in light "more recessed" runs `bg-white` -> `bg-gray-100`, in dark it runs `#1f1f1f` -> `#151515`, and the fold points `gray-100` at the LIGHTER dark tone `#262626` - a folded conversation floor would rise above the chrome it is meant to sink into. Stating both ends keeps the light direction right and the dark values exactly what shipped. The mail window and the inbox import them, so the six windows cannot drift apart |
