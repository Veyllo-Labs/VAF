...
class WorkflowEngine:
    ...
    def __init__(
        ...
        # Resolved from the SCOPE when no name was passed, never substituted: a literal here
        # would also pre-empt the shared rule in assign_declared_identity, arriving truthy so
        # the one place that knows it never gets asked. A workflow started by a tenant whose
        # name is not in the metadata must not run as the owner.
        from vaf.core.config import resolve_caller_username
        self.username = resolve_caller_username(username, user_scope_id)
                ...
                def _inject_user_scope(tool_name: str, a: Dict[str, Any]) -> None:
                    ...
                    if self.user_scope_id is not None or self.username:
                        if tool_name in ("memory_save", "memory_search"):
                            ...
                            a["username"] = self.username
                            a["user_scope_id"] = self.user_scope_id
                        elif tool_name in ("find_whatsapp_messages", "read_whatsapp_chat", "whatsapp_call"):
                            a["username"] = self.username
                            a["user_scope_id"] = self.user_scope_id
                        elif tool_name in ("list_contacts", "get_contact", "create_contact", "update_contact", "delete_contact"):
                            a["username"] = self.username
                            a["user_scope_id"] = self.user_scope_id
                        elif tool_name in ("inbox", "read_mail", "find_mail", "mark_mail_answered", "label_mail", "list_email_accounts", "send_mail", "reply_mail", "forward_mail", "archive_mail", "delete_mail"):
                            a["username"] = self.username
                            a["user_scope_id"] = self.user_scope_id
