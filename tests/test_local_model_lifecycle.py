...
def test_a_child_we_gave_up_on_is_parked_for_reaping():
    ...
    assert "_ABANDONED_CHILDREN.append(self.process)" in stop, (
...
def test_both_reuse_paths_treat_503_as_loading():
    """llama-server answers 503 while it maps a multi-GB GGUF. Killing it then and starting
    another one is the shape of the incident. Both health checks in start_server must wait."""
    src = _source("vaf/core/backend.py")
    body = src[src.index("def _start_server_locked"):]
    body = body[:body.index("\n    def ", 10)]
    assert body.count("503") >= 2, (
        "both the direct health check and the PID-file check must handle 503"
    )
    pid_block = body[body.index("if os.path.exists(self.pid_file)"):]
    assert "503" in pid_block, "the PID-file path still kills a server that is only loading"
