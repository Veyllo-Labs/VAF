...
- **In-chat image uploads** are stored here too, under `attachments/`, and emit a `file_created` event — so attaching an image makes the workspace chip appear (the folder now exists), just like an agent-created file. See the vision section in `docs/llm/API_INTEGRATION.md`.
