...
- Two named boundaries of the glances: the mail lane reads the v2 store (`mail.db`: the address in From, To or Cc; Junk, Trash and Drafts left out; direction from the headers and the Sent folder) and, for a legacy per-username user (username set, no scope), only the legacy `email_sync.db`, which is INBOX-only and therefore inbound-only (`vaf/mail/tool_bridge.messages_for_address_merged`); and Discord rows are written under the literal admin identity, so the Discord lane runs for the local admin only.
...
- **Edit form** (**Add contact** in the header, **Edit** in the menu or a card): name, the channels editor (WhatsApp, phone, Telegram, email, Discord; a phone number counts as WhatsApp once the contact may write to the agent), company, role, tags as a comma-separated field with suggestions and the personal file fields. Whether the person may reach the agent is switched only in the detail header, behind its confirmation; a new record starts closed.
