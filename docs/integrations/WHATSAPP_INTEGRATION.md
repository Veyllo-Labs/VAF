...
| `reply_window_hours` | number | How long a number the agent wrote to may reply without being a contact (Front Office). Default `72`; `0` switches the window off. The window is measured from the newest outbound message in the per-user message store; a reply the contact sends inside it extends it for the reply lane, and a message that arrives after it closed (stored for you nonetheless) opens nothing |
...
- **Open conversations (reply window)**: When the agent writes to a number (`send_whatsapp(to_phone=...)`, or a reply in a Front Office chat), that number may answer for `reply_window_hours` (default 72) even without a contact record, and with a record whose "Can reach your assistant" is off (the sync creates such a record for every named chat, so the flag closes no window the agent opened; it keeps a person out of an open Front Office, see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md)). The message is handled in Front Office; the prompt states that there is no contact record and that the agent started the conversation, and the owner is informed through the usual back-channel. Every agent reply extends the window, and so does the contact's answer inside it (an accepted answer can always be answered); a one-sided sender the agent never answers expires, and an inbound that arrives after the window closed is stored for you but opens nothing (`conversation_open_until`). A message YOU send from the dashboard's compose box opens no window either: it is stored as an outbound row under `OWNER_SENDER` (`channel_message_store`), and the window reads only what the agent sent, so a contact you wrote to by hand stays read-only for the agent. The decision is `channel_ingress_policy.evaluate_ingress(..., conversation_match=True)` with reason `open_conversation`, computed from the newest outbound row in the message store (`channel_message_store.last_message_ts`).

The bridge builds the allowed set from the config whitelist plus all WhatsApp/phone channel values from contacts with "Can reach your assistant" enabled, read from the user's OWN contact book only (their scope file, then their legacy per-username file; `contacts_store.front_office_endpoints`), and adds the reply window on top. The local admin's book is never consulted for another user: it used to be, which let a person the admin had cleared write to any user's agent number. A reply-lane call that arrives without a scope resolves it from the account name (`scope_id_for_username`) instead of defaulting to the admin's. The same three answers gate the reply lanes: the headless runner's reply and the owner delivery go out only to a registered number, a Front Office contact or an open conversation (`_is_reply_allowed`); an unresolved `@lid` matches none of them.
...
- **Conversation header**: name, badge, number and a one-line explanation of what the agent does with this chat; **Add as contact** on a conversation or read-only chat creates a contact with "Can reach your assistant" (`POST /api/contacts`); when the number is already in the contact book (the WhatsApp sync creates such records without the flag), the header says so instead and shows a **Let the agent reply** switch: switching it on asks once whether the agent may talk to this person in Front Office mode and then sets the flag on the existing record, switching it off clears the flag without a question (`PATCH /api/contacts/{id}`; the dashboard payload carries `contact_id` and `contact_name` per chat). **Load older messages** is a pill at the top of the message list, where scrolling up lands, not a header button. The search over chats and contacts sits at the top of the chat list; the search field in the conversation header searches inside the open chat. Where the agent answers, the footer names the mode (full agent, Front Office) and, for the owner's chat, the message count until the next Memory Learning; where the person writes themselves, the compose box takes the footer's place.
- **Compose box and Composer** (where the agent does NOT answer: a read-only chat, or every chat while `inbound_to_agent` is off; owner, contact and conversation chats are the agent's, and an unassigned `@lid` has no address): under the conversation a text field with **Send** (Enter sends, Shift+Enter breaks the line; the field grows with what you type to three lines and scrolls beyond that without a visible bar, the Composer's instruction field likewise) sends your own words from the agent's number, `POST /api/whatsapp/send` (`chat_id`, `text`); the bridge's sender loop stores the row and the pane reloads. The message is stored under `OWNER_SENDER`, so it opens no reply window: the person's answer stays a read-only message for you. To the right of the conversation the **Composer** drafts into that text field, the same assistant as the mail window's on the shared lane (`vaf/core/composer.py` with the `CHAT` profile, `vaf/core/composer_lane.py`; design and containment in [EMAIL_CLIENT.md](EMAIL_CLIENT.md#mail-composer)): say what the reply should say or what to pay attention to, **Write a draft** reads the stored chat (newest inbound message first, your side labelled so the model matches how this number writes, memory keyed on your instruction, plus what the agent learned in this chat if it ever answered here), **Rewrite my text** works on what you typed and reads no chat messages (it still knows what the agent learned in this chat), follow-ups refine the last draft, **Stop**, **Undo**, and a result line that says how much of the chat was read and whether it had a sample of your tone (`POST /api/whatsapp/composer`, SSE frames identical to the mail route: `meta`, optional `notice`, cumulative text, `error` or `end`). One model call, no tools, nothing sent: you press Send. The panel is offered while `mail_composer_enabled` is on (`composer_enabled` in the dashboard payload); on a phone it stacks under the conversation. Switching chats keeps each chat's box and Composer exchange in the window's memory (a state swap, no request) and brings them back when you return; a draft or a send still running for the chat you left is bound to that chat, so its outcome lands in that chat's stash rather than in the one on screen; closing the window drops them all.
...
- **Chat list**: Built from (1) Node’s chat list (Baileys), (2) `chat_activity` (incoming/outgoing activity), (3) Front Office contacts (contacts with "Can reach your assistant" and a WhatsApp channel) so that chats appear even before Baileys has synced them, and (4) the per-user message store, the persistent inbox: every chat it holds is a row. **Deduplication:** Each contact is shown once; the same person as E.164 and as @lid is merged into one row when LID→E.164 is known (config or Node). Phone numbers are normalized to a single leading `+` to avoid duplicate entries (e.g. `++49...`). The **message count** shown per chat is the **store's count** (the number the inbox and the agent's `inbox` tool show), and each row carries the person's own state from the same overview the inbox reads: `unread`, `waits`, `waits_reason`, `answered_by_agent`, `done`, `last_preview`, `preview_from` (see [INBOX.md](INBOX.md)). The **Conversation** badge and `reply_window_until` are computed from those rows with the bridge's reply-window rule; the dashboard asks the bridge for the chat list and the names only, never for the window. **Contact names** are resolved from the contact list; matching uses canonical phone form (0-prefix German numbers, e.g. `0152...`, are treated as `+49...`) so names appear even if the contact was stored as `0152...` and the session uses `+49152...`.
