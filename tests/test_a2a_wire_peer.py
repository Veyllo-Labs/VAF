...
def test_mcp_tools_list_names_every_room_verb(peer):
    ...
    assert {t["name"] for t in tools} == {
        "a2a_join", "a2a_rooms", "a2a_read", "a2a_verify", "a2a_members", "a2a_wait",
        "a2a_say", "a2a_answer", "a2a_report", "a2a_leave", "a2a_howto",
        "a2a_files", "a2a_fetch", "a2a_push", "a2a_react"}
    for tool in tools:
...
def test_vafs_own_mcp_client_drives_the_guest_bridge(peer, server, capsys):
    ...
    try:
        ...
        assert "a2a_say" in names and len(names) == 15, names
        before = len(server["state"]["submissions"])
