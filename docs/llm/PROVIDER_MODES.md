...
Backend selection order: a configured API provider (`api_backend`) wins over the local server (:8080), which wins over the in-process library (`agent.py.load_model`).
...
|----------|---------------------------|-------|-----------|
| **DeepSeek** | `reasoning_content` is streamed alongside `content`, and must be passed back as a **separate field** on the next call (else `400`); answer is often in `reasoning_content` only | `api_backend.py` (stream ~87-133), `_prepare_messages` (`agent.py`), `clean_history` (`coder.py`) | internal notes |
...
| **DeepSeek** | `tool_choice="required"` and specific function-forcing are rejected with a 400 (both `v4-flash`/`v4-pro` are internally reasoning models). Universal downgrade to `"auto"` for every manager-routed caller; the HTTP-direct coder path keeps its own guard | `api_backend.py` (`APIBackendManager.chat_completion`, ~857-871), `coder.py` (`tool_choice` downgrade) | internal notes |
| **DeepSeek** | deprecated model names (`deepseek-chat`/`-coder`/`-reasoner`) auto-migrate to `deepseek-v4-flash` on config load; `-reasoner` also dropped because it rejects `tool_choice` | `config.py` (~368-374) | internal notes |
...
| **Veyllo** | OpenAI-compatible via `base_url` = `https://api.veyllo.app/v1` (configurable via `veyllo_base_url`); `veyllo-chat` is multimodal, so one model serves both chat and image input - no separate vision provider is required when Veyllo is primary. Tool exchanges whose ids VAF minted itself (`call_synth_` prefix: text-recovered calls, id-less streams) are folded into plain text pre-send for this provider (gateway compatibility); cross-provider failover is suppressed whenever the outbound history contains tool messages. Speaks DeepSeek's thinking dialect: inline `<think>` blocks in replayed assistant messages are restored to a separate `reasoning_content` field pre-send, exactly like the DeepSeek row above. The gateway's "reasoning_content ... must be passed back" 400 is NOT what that restore fixes, though it was read that way when it was written. Direct probes return that identical text for two unrelated causes, with the field present just as readily as without: a replayed tool_call id the gateway never issued, and an assistant turn appended after a tool result with no user turn behind it. Measured one variable at a time with a `call_00_` id the gateway had just issued: `tool` last, `tool, system` and `tool, user` all answer 200, `tool, assistant` and `tool, assistant, system` answer 400, and `tool, assistant, user` answers 200 again. So the normal loop step and a post-tool nudge are fine; what is refused is the agent speaking again after a tool result without the conversation being handed back. That shape is what a self-correction retry builds (result grounding), and `_close_assistant_after_tool_result` repairs it by converting the trailing mid-run system message into a user turn for this provider - the same mechanism `consolidate_system_messages` applies for strict local templates, whose docstring names Qwen's "Assistant response prefill is incompatible with enable_thinking". The two mechanisms COMPOSE: the restore runs first, the synthetic-id fold second, and the fold carries `reasoning_content` on the folded assistant message (keeping the turn even when the think block was its whole content) - dynamically, field present only when reasoning exists. The fold used to rebuild from content alone; a text-recovered tool call inside a thinking turn then lost its reasoning on replay and 400ed with that same message, while a parallel turn without a recovered call ran clean (live incident 2026-08-21) | `OpenAIProvider` (base_url), `api_backend.py` factory + `_messages_have_provider_bound_tool_calls`; `agent.py` `_downgrade_synthetic_tool_exchanges` + the DeepSeek-family `reasoning_content` restore (both in `_prepare_messages`) | internal notes |
...
| **All API (chat while a sub-agent runs)** | while a sub-agent genuinely runs for the session, the main agent gets a SUB-AGENT ACTIVE prompt block (keep replies light, no re-delegation, hands off the workspace) and typing/sending stays unlocked while the sub-agent runs - gated on the MAIN provider != `local` AND an initialized `api_backend` (closes the API-init-failure fallback hole), plus NOT `_background_run` (automations) and NOT front-office. On `provider=local` the block is simply absent (chatting itself is not newly blocked): the single llama server would serve two inferences at once. Hybrid main=local + subagent=API stays gated OFF (the gate keys on the main provider - it is the main agent's extra inference that hits the llama server). Kill-switch: `subagent_concurrent_chat_enabled` | `system_prompt.py` (`<subagent_active>` block), `agent.py` (`get_live_session_subagents`, re-delegation guard), `web_server.py` (scoped stop) | [CONFIG_SCHEMA.md](../setup/CONFIG_SCHEMA.md), [SUBAGENT_IPC.md](../agents/SUBAGENT_IPC.md) |
| **All API (provider registry / SSOT)** | the provider SET and its endpoints live in ONE place: `vaf/core/provider_registry.py` (`ProviderSpec` per provider - factory kind, SDK/coder/discovery URLs with their genuinely different values per lane, key requirement, vision capability). Consumers read from it: the `APIBackendManager` factory, `coder_api_providers()`, live model discovery (sync `list_models` and the async web handler), the vision predicate (one shared implementation, previously three diverged copies), and the CLI settings menus. Provider-specific BEHAVIOR stays as gated code branches per this page's principle. CI-enforced by `tests/test_provider_registry_sync.py` (registry == `PROVIDER_MODELS` order, coder/discovery coverage, web-UI list sync) and `tests/test_provider_factory_pinning.py` (byte-stable factory results and error messages) | `vaf/core/provider_registry.py` + consumers | this page |
| **All API (coder endpoint map)** | the coder talks raw OpenAI wire format and resolves its endpoint from `coder_api_providers()`, which is built from the provider registry - it covers EVERY provider in `PROVIDER_MODELS` by construction (additionally CI-enforced by `tests/test_coder_provider_map.py`); an unknown API provider errors loudly instead of falling back to the local `:8080` branch, so a misconfigured API provider fails visibly rather than being served by the local model | `vaf/tools/coder.py` (`coder_api_providers`, run() gate), `vaf/core/provider_registry.py` | [CODER_ARCHITECTURE.md](../agents/CODER_ARCHITECTURE.md) |
...
|------|----------|-------|-----------|
| Default model | `model: "auto"` → **VRAM-adaptive Qwen3.5**: 4B (`unsloth/Qwen3.5-4B-GGUF`) at ≤ 10 GB, 9B (`unsloth/Qwen3.5-9B-GGUF`) above; **quant leaves room for desktop + KV cache** - 4B: Q4_K_M (<8 GB) / Q6_K (8) / UD-Q8_K_XL (9–10); 9B: Q5_K_M (11) / Q6_K (12–15) / Q8_0 (16–19) / UD-Q8_K_XL (20–23) / BF16 (≥24) | `gpu_detection.recommended_default_model`, `agent.py`, `backend.py.get_model_path` | internal notes |
| Server launch | `--jinja` (the GGUF template parses tool calls), KV cache `q8_0`/`q4_0`, `n_ctx` floor 32768 | `backend.py` (~600-650) | internal notes |
| CUDA | auto-install of CUDA `llama-cpp-python` only on the in-process library path (`auto_install_gpu`, default `true`) | `agent.py.load_model` | internal notes |
| Server vs library | `force_server` / Windows default / `use_server` | `agent.py.load_model` | internal notes |
| OpenAI-compatible URL (browser agent / `APIBackendManager` local) | Defaults to VAF's own llama-server via `get_llama_server_url("/v1")` (port 8080, Docker/env-aware) - **not** Ollama's `:11434`. The browser agent's `VAFLLMBridge` uses this path; pointing it at Ollama's `:11434` instead would fail with a connection error since VAF serves on 8080. An explicit `local_api_url` (e.g. a real Ollama) overrides. | `api_backend.py` `_create_provider`, `browser_agent.py` `VAFLLMBridge` | - |
| Voice call lane (live call) | The voice lane is configurable (`voice_agent_provider`/`voice_agent_model`, admin-only): empty = ride the main provider; `local` = a DEDICATED voice GGUF (chosen from downloaded models in Settings > Voice; default Gemma 4 E4B, fetched on selection) - the ONE llama server **swaps models** (voice model during the call, main model while a delegated task runs; `start_server` reuse is model-aware and the runner has a swap-back belt so a main turn never runs on the voice model); an API id = the call runs on that API. On any local flavor `voice_agent.is_exclusive()` is True (except dedicated-local + API main): availability is an honest probe of `GET :8080/v1/models`, a reply is one non-streaming `POST /v1/chat/completions` with `chat_template_kwargs: {enable_thinking: false}` (a local reasoning model otherwise burns the whole voice token budget on `reasoning_content`; templates without the variable ignore it), and voice turns pause while the main agent runs a delegated task (frontend mute + `voice_call_error "busy_local"` belt before STT) - never two servers, never two concurrent inferences. | `vaf/core/voice_agent.py` (`_resolve_backend`, `is_exclusive`, `dedicated_local_model`), `vaf/core/voice_model.py`, `backend.py` (`ensure_local_model`, model-aware `start_server`), `headless_runner.py` (swap-back belt), `web_server.py` (`voice_call_started` `exclusive` flag, busy belt) | [VOICE_AGENT.md](../agents/VOICE_AGENT.md) |
| Local vision (`vision_provider = local`) | The llama server launches with the model's mmproj projector (`backend.resolve_mmproj_for`: explicit `vision_local_mmproj` ref, else `mmproj-F16.gguf` from the model's known repo; downloaded WITHOUT the chat-model self-heal - a substituted chat GGUF as `--mmproj` would kill the server). The mmproj size is counted into the VRAM n_ctx cap; `--image-max-tokens 1024` bounds per-image context cost. The existing vision path then just works: `vision_infer` already POSTs OpenAI `image_url` data-URIs to `:8080` via `APIBackendManager("local")` - it only ever failed because the projector was missing. `_model_supports_vision("local")` probes the running server's `/v1/models` for image support (honest capability). Quirk: with an mmproj loaded, llama-server force-disables ctx_shift/cache_reuse - acceptable, VAF manages context itself. Vision activates on the next model (re)start after enabling. During a dedicated-voice-model call window the server may hold the voice GGUF: image describe degrades gracefully to the "analysis unavailable" marker rather than forcing a mid-call swap. | `backend.py` (`resolve_mmproj_for`, launch args, VRAM math), `vision_infer.py` (`_model_supports_vision`) | [API_INTEGRATION.md](API_INTEGRATION.md) |
...
| Reasoning models - internal LLM calls (`query_llm`) | The local server splits output into `reasoning_content` (chain-of-thought) and `content` (answer); `query_llm` reads `content`. Tool-side utility completions (web_search page summaries/synthesis) now send `chat_template_kwargs: {enable_thinking: false}` on the local lane - the voice-lane precedent - because live evidence showed a reasoning model burning the ENTIRE small budget on `reasoning_content` and returning empty `content` every time (`finish_reason="length"`, `reasoning_len=2691` at `max_tokens=600`); a chain of thought adds nothing to a 3-sentence page summary. Belts stay: generous `max_tokens` + real `timeout`, and the `reasoning_content` fallback (logging `finish_reason`/`reasoning_len`) for models/templates that ignore the flag. The same `finish_reason` is now recorded for EVERY lane, not only when the content came back empty: it appears as `cut=` in `logs/usage_*.log` whenever the output limit ended a response. The main chat lane carries no belt of its own yet, which is what that number exists to size. | `tools/base.py` `query_llm`, `tools/search.py` (synthesis) | - |
| Shared one-shot completion path | ONE implementation for "one prompt in, one text out": `vaf/core/completion.py` `complete()`, consumed by `Agent.complete`/`CoreAgent.complete`, `BaseTool.query_llm` (delegation), the git/debug/generate CLIs, the coder template classifier and the memory/attachment RAG answerers. Its gates, uniform for every consumer: the API lane streams via `APIBackendManager` (no tools ever) with the metadata-frame filter and error-sentinel-to-None; the local lane posts ONE non-streaming request to `Config.get_llama_server_url` with `chat_template_kwargs {enable_thinking: false}` and NEVER starts the server (a dead server is None, not a spawn - the ONE-llama-server invariant, auditable by the module's import list); `<think>` blocks are stripped by default, an unclosed block truncates. Remaining direct `chat_completion` callers are streaming/multi-turn lanes, frozen shrink-only in `tests/test_completion_call_baseline.py`. | `vaf/core/completion.py`, `agent.py`/`framework.py` `complete`, `tools/base.py` `query_llm` | - |
