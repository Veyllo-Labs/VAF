...
3. **Plan gate (main agent only)** — for `write`/`dangerous` tools (except `python_sandbox`), if no plan exists in working memory the call is blocked with a `[PLAN REQUIRED]` message asking the agent to write a short plan first; satisfied in the same turn, with a loop-cap escape so it never hard-locks. Skipped for sub-agents and non-interactive runs. See [CONTEXT_MANAGEMENT.md](../memory/CONTEXT_MANAGEMENT.md).
3a. **Proactive-reply mutation gate** — while the turn is a pickup of the user's reply to a tracked background question and that reply is not a CLEAR affirmative, stored-state mutations (`update_automation`, `delete_automation`, `create_automation`, workflow/tool builders) and destructive delegation (a sub-agent task containing delete verbs) are answered with a `[CONFIRM REQUIRED]` result: acknowledge and ask one confirming question instead of acting. Kill-switch: `proactive_reply_mutation_gate_enabled`. Added after a live incident in which a misattributed "nein bitte nicht" mutated an automation and delegated a file deletion.
3b. **Ask-first gate (drain turns)** — once the agent's final reply asked the user a blocking question (classified by the autocontinue question classifier), synthetic runner-drain turns may deliver results and read, but new `write`/`dangerous` tools and delegations are answered with an `[AWAITING USER]` result until a real user message arrives. Kill-switch: `ask_first_drain_gate_enabled`. The drain's retry instruction is likewise replaced by a status report while a question is pending.
...
8. **Identity injection** - the keys named in the tool's `identity_kwargs` are ASSIGNED from the agent's current session (`_current_user_scope_id` / `_current_username` / `_current_user_role`), overwriting anything the model supplied under those names. Session and workspace plumbing (`_session_id`, `_session_workspace`, `_agent`) stays per-tool and is not part of the declaration. `ask_user` is the one deliberate exception: it injects identity only for background runs, because a non-admin's private question would otherwise be delivered to the ADMIN's messenger. If arguments still violate the schema after repair, the tool is not run and a localized `Tool Error: invalid arguments for '<tool>': <detail>` is returned. See [TOOL_INPUT_REPAIR.md](TOOL_INPUT_REPAIR.md).
