...
- **A password in a message**: when the agent stores a credential it read in one of your messages (`store_credential`), the bridge tries to delete that message in the Telegram chat. It is an attempt, not a promise: a bot may delete a person's message in a private chat only for 48 hours, the Bot API can refuse, and a message that arrived before the bridge last started is not found, because the bridge keeps the chat's recent text messages in memory only - it needs them to find the message at all, because a debounced burst is one turn and only its text says which message carried the value (`_remember_inbound`, `_forget_listener` in `vaf/api/telegram_bridge.py`). The outcome is logged in `telegram_reply_<date>.log` (`CREDENTIAL_MESSAGE deleted` / `not deleted`, with chat and message id). The value is replaced everywhere VAF keeps the chat as well ([CONNECTIONS.md](CONNECTIONS.md)).
...
- [SPEECH_FEATURES.md](../web-ui/SPEECH_FEATURES.md) - TTS/STT technical details
- [DOCKER_SERVICES.md](../setup/DOCKER_SERVICES.md) - Container setup
- [MEMORY_SYSTEM.md](../memory/MEMORY_SYSTEM.md) - Memory and user scopes
