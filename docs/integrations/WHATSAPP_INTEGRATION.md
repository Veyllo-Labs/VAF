...
- **Conversation header**: name, badge, number and a one-line explanation of what the agent does with this chat; **Add as contact** on any chat the book does not know yet creates the record and nothing more (`POST /api/contacts`): adding somebody is not the same as handing them a pass that outlives the Inbound switch. When the number is in the contact book, the header says so and shows **Let the agent reply** as three positions, **allowed** / **the channel decides** / **blocked**: **allowed** asks once whether the agent may talk to this person in Front Office mode, the other two are set without a question (`PATCH /api/contacts/{id}` with `assistant_access`; the dashboard payload carries `contact_id`, `contact_name` and `contact_access` per chat). A two-way switch could only ever take a decision back, never refuse a person, which is why there are three. **Load older messages** is a pill at the top of the message list, where scrolling up lands, not a header button. The search over chats and contacts sits at the top of the chat list; the search field in the conversation header searches inside the open chat. Where the agent answers, the footer names the mode (full agent, Front Office) and, for the owner's chat, the message count until the next Memory Learning; where the person writes themselves, the compose box takes the footer's place.
