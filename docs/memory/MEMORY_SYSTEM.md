...
- **Scope:** Learning is per user (`user_scope_id`) and, inside a messenger chat with a contact, per chat: the main user's own conversations (Web UI and their own channel chats) compact into the general lane; a chat the agent answers for a contact (an allowed contact, or anybody while that channel's Inbound is open, on WhatsApp, Telegram and Discord alike) compacts into that chat's own namespace, which the general lane never reads (see [Chat memory namespaces](#chat-memory-namespaces-what-a-contact-chat-teaches-stays-in-that-chat)). Telegram relay contacts get no agent answer and no learning, and messages the bridges only store never reach a session.
...
- **When:** Compaction runs for the main user's own sessions (the Web UI chats and the main user's own Telegram, WhatsApp and Discord chats: a channel task without the `from_contact` flag) into the general lane, and for a chat the agent answers for a contact (a task with `from_contact` and no `relay`: the ingress reasons `contact_allowed` and `front_office_open` on WhatsApp, Telegram and Discord, and the Telegram contact entries) into that chat's own namespace, never the owner's general lane. The rule is channel-agnostic: `ChatNamespace.from_task` reads the two flags and takes the channel from the task's `origin_channel` or the session id's prefix, so an accepted Discord direct message (`discord_<id>`, admitted through the admin's Inbound) learns into its own namespace exactly like a WhatsApp or Telegram contact chat. The two are one gate: a contact task exists only when the agent answers there, so learning from a person's chat is bound to "the agent may answer in this chat". The runner decides with `ChatNamespace.from_task` and enqueues the same compaction task with the namespace keys. After every chat task, the headless runner checks whether that session has reached the compaction interval (number of user turns since last compaction). Default: every **15 user turns** (`memory_compaction_interval`). The count is cumulative per session; only messages with role `user` are counted.
- **What:** A single non-streaming LLM call with a prompt like “Store durable memories now. Write any lasting notes to memory/{date}.md. Output MEMORY: \"...\" lines or NO_REPLY.” The prompt includes a **conversation excerpt (user and assistant messages only; no system prompts or tool calls)**. Reply format: `MEMORY: "content" [tag1, tag2]` (tags optional but recommended) or NO_REPLY. Parsed lines are ingested with metadata: `type=conversation`, `source=memory/{date}`, `tags` (orange in memory graph). Then the user-profile summary cache is refreshed. Max reply length: `memory_compaction_max_tokens` (default 4000). With `chat=ChatNamespace(...)` the same call learns about the person in ONE messenger chat instead: the prompt asks for facts about that person and what was agreed with them (and forbids facts about the owner, the assistant or third parties), the transcript is the stored session labelled with the person's name, the facts land under `source=chat/<session id>` with `chat_key`, `chat_channel` and `chat_label`, and the user-profile cache is NOT refreshed, because it feeds every system prompt and must never carry a contact's facts.
...
- **Model-independent gates (`_apply_fact_gates`, between parse and ingest):** length bounds 15-500 chars, junk-marker rejection (NO_REPLY echoes, `<think`, injected-context markers, meta commentary, nested `MEMORY:` lines) and a hard cap of 12 facts per run - a weak model cannot flood the store. Rejections are logged per fact (`COMPACTION_FACT_REJECT` with reason). Additionally, each surviving fact runs a **dedup check** before ingest (chunk-level similarity search, threshold 0.95, same primitive as the auto-capture lane and the same singleton embedding service): near-identical existing facts are skipped and logged as `COMPACTION_DEDUP_SKIP`. The check is a pure cosine search (`hybrid=False`, as the `memory_save` check): the hybrid fusion returns rank values no threshold can read, and its lexical lane admits any row sharing one token, which in a small namespace would have made the first fact the only fact. On a chat run the check looks only inside that chat's namespace, so a fact about the owner never suppresses a contact's fact and the other way round. Dedup is best-effort and never blocks ingestion on errors.
...
- **Where:** Logic in `vaf/memory/rag.py` (`run_session_compaction_sync`); triggered from `vaf/core/headless_runner.py` (after each chat, or enqueued as a separate task when using a local LLM so only one LLM call runs at a time). State per session: `~/.vaf/compaction_state.json` (last compaction turn per `session_id`). A chat run receives its transcript through `conversation=` from `session_dialogue_excerpt` (the stored session, never the live agent history, which holds the Front Office wrapper around the person's message) and skips without a model call when there is none.
- **Contact chats (WhatsApp, Telegram, Discord):** what the agent learns in a chat with a contact is about that person and what was agreed with them, and it stays in that chat: `source=chat/<session id>`, retrievable only by a turn inside that chat and by the WhatsApp Composer for that chat, never by the owner's Web UI, automations, thinking runs, rooms, voice or the mail Composer, and never copied into the user-profile cache. It is visible in the Memory graph as one node per chat and deleted hard, as one unit, from there. This replaces the earlier rule that contact conversations were never written to memory at all: the rationale is the same (a person's words belong to that person's conversation), the mechanism is a namespace the rest of the product cannot reach instead of no memory. Messages the bridges only store without an agent run (rejected senders, a channel with inbound switched off, the owner's manual sends) still never reach a session, so they cannot be learned from; Telegram relay contacts are answered by nobody and learn nowhere; the admin's own Discord direct message carries no `from_contact` and stays in the general lane, while a stranger's, admitted through the Discord Inbound, is a contact chat with a namespace of its own.
...
```python
{
    ...
    "metadata": {
        ...
        "chat_channel": "whatsapp | telegram | discord, with chat_key",
...
- **The lane is set at ingest and an update cannot move it.** `RagPipeline.ingest` strips the
  namespace keys from every writer but the chat compaction (`keep_namespace`), so a request body
  cannot plant a row in a namespace, and `update_memory` pins `source` and the three namespace keys
  of a chat memory and strips them from any other memory (`pin_namespace`). The graph groups a
  memory under a chat node only when its SOURCE is a chat lane, the same rule the delete uses.
