...
| `front_office_changed` | Front Office was switched on or off for a messenger channel in Settings: on, everyone who writes there is answered in Front Office mode and the channel's contacts in the book are allowed. `channel` the platform, `username` who switched it, `detail` the new state (`on, N contacts granted` or `off`). One event per channel that really changed; a write that changes nothing records nothing | `vaf/api/front_office_routes.py` (the switch in Settings, Connections) |
