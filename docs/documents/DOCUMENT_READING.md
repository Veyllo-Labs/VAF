...
The agent uses the `cloud_storage` tool: first `browse` to locate the file and get its `file_id`, then `read` with that `file_id`. The file is downloaded to a temporary path, text is extracted with the Librarian, and the temp file is deleted. Supported: PDF, Word, Excel, PowerPoint, and native Google Docs/Sheets/Slides (exported to text or CSV). See [CONNECTIONS.md](../integrations/CONNECTIONS.md#cloud-storage-integration) for setup.
