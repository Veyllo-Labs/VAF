...
- **Agent activity:** Right column shows the same events in a compact, scrollable list with a fixed max height so the layout stays balanced.
...
- **Token Expiry/Revocation**  
  If the agent can no longer access GitHub, disconnect the account (trash icon) and reconnect using the Device Login.
