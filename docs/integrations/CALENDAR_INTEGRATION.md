...
- **Per-User Isolation**: Calendar access is scoped by `user_scope_id` (and username); credentials and data follow the same isolation as email (see [USER_ISOLATION.md](../security/USER_ISOLATION.md)).
...
- **Daily calendar check (automatic)**: When a calendar is connected, the frontend calls **POST /api/calendar/ensure-daily-check-automation** (on opening the Automation calendar or when calendar is connected in Settings). If the user does not already have a "Daily calendar check" automation, one is created with the current user's `user_scope_id`; it appears in **Settings → Automations** and in the Automation popup (and in `vaf automation list`). It runs daily at 08:00 by default; the user can change the time or disable it. The automation runs in the **task owner's scope** (see [AUTOMATIONS.md](../platform/AUTOMATIONS.md)), so calendar, memory, and messaging use that user's data. The prompt instructs the agent to: fetch the next 24–48 hours of events, analyze importance, and for important events either schedule a one-shot reminder via `schedule_reminder` (delivered verbatim at the stored time on the user's main messenger - stored data, no agent run) or send immediate preparation help via `send_to_user`. `create_automation` is deliberately NOT available inside automation runs (runaway guard); the narrow reminder lane replaces it for this use case. See [AUTOMATIONS.md](../platform/AUTOMATIONS.md).
...
- [USER_ISOLATION.md](../security/USER_ISOLATION.md) – User-scoped credentials and data; calendar uses the same model as email.
- [WEB_UI.md](../web-ui/WEB_UI.md) – Settings → Connections; Calendar category and Calendar Dashboard.
