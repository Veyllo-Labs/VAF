...
- If you have stale fake entries in config, remove them manually: open `whatsapp_config.lid_to_e164` in your VAF config and delete entries where the key (before `@lid`) has the same digits as the value (E.164 phone number). Example — **remove**: `"491701234567@lid": "+491701234567"`. **Keep**: `"491702345678@lid": "+491703456789"` (digits differ → genuine LID).
...
If a **known** Front Office contact (e.g. Bob) uses a chat that WhatsApp sends as **@lid** (e.g. `491702345678@lid`) and the Node never sends `fromE164`, the bridge will **reject** their messages after the security fix (`REJECT unresolved @lid from=491702345678@lid`). To allow that contact again, add a **manual LID→E.164 mapping** in config so the bridge can treat that LID as their phone number:

1. In **whatsapp_inbound.log** note the rejected JID (e.g. `491702345678@lid`).
2. In your VAF config (`~/.vaf/config.json` or `%APPDATA%\\vaf\\config.json`), under `whatsapp_config`, add or extend `lid_to_e164` with that JID as key and the contact’s **real E.164 number** (as in Front Office) as value. Example: `"lid_to_e164": { "491702345678@lid": "+491703456789" }` (use Bob’s actual number from Contacts).
