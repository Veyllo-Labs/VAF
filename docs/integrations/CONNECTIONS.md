...
| **Let the agent reply** (`assistant_access`) | Three positions, because the middle one is a state and not the absence of one. **Allowed**: the agent answers this person on every channel where you have their address, even with that channel's Inbound off. **Blocked**: the agent answers them nowhere, even with Inbound on. **The channel decides** (the state a new or synced record carries): they are answered exactly while their channel's Inbound stands open. The assistant processes them in your context (like a front office for you), not as a separate user. |
...
- **Detail header**: avatar, name, `company · role`, the status (click to type, suggestions from the known values), the tag chips (remove, **Add tag** inline), the actions **Open WhatsApp chat** / **Open Telegram chat** (shown when the overview's `endpoints` carry a key for that channel: the Contacts window closes and the channel dashboard opens on that chat; when the dashboard has no stored chat under that key it selects its first chat and says "No chat is stored here for this contact"), **Note** (focuses the composer), **Event** (opens the inline event form), a menu with **Edit** and **Delete contact**, and the **Let the agent reply** switch with a one-line hint of what the current state means (on, off, or on while the Front Office door for this person's channels is closed in Connections, so they are still turned away). Switching it on asks once, in the same dialog the WhatsApp dashboard uses, whether the agent may answer this person in Front Office mode; switching it off asks nothing.
