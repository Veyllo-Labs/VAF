...
### Fixed
- Calendar window, dark mode: the paper-dot texture behind the to-do list and the notes kept its light-mode grey (an inline hex the folding palette never sees), so the dots outshone the text. The dot colour is the palette's own variable now and folds with the theme; light mode is unchanged.
