...
You **do** need a commercial license if you want to do any of the following **without**
AGPL copyleft:

- embed VAF in a **closed-source / proprietary product** you distribute;
- offer a **modified VAF as a hosted SaaS** without publishing your modified source
  (AGPL Section 13);
- combine VAF with proprietary code in a way that would otherwise make that code a
  derivative work subject to the AGPL;
- redistribute VAF under terms other than the AGPL.

> Quick test: **"Do I want to keep my own changes to VAF closed?"** If yes, you need a
> commercial license. If no, the AGPL is all you need.
...
- Optional **warranty and indemnification** terms (not available under the AGPL's
  "as-is" Sections 15–16).
...
This separation is deliberate: it keeps the open source edition fully functional for the
community while giving organizations concrete reasons to buy.

### Pricing & how to obtain one

Commercial licensing is offered **on request**, priced to the use case (team size,
deployment, support tier). To get a quote or ask a question:

- Email **legal@veyllo.io** with a short description of your intended use.
- Or visit **https://veyllo.io**.
...
AGPL-Copyleft tun willst:

- VAF in ein **proprietäres / Closed-Source-Produkt** einbetten und weitergeben;
- ein **verändertes VAF als gehostete SaaS** anbieten, ohne deinen veränderten Quellcode zu
  veröffentlichen (AGPL Abschnitt 13);
- VAF mit proprietärem Code so kombinieren, dass dieser sonst zu einem abgeleiteten,
  AGPL-pflichtigen Werk würde;
- VAF unter anderen Bedingungen als der AGPL weitergeben.

> Schnelltest: **„Will ich meine eigenen Änderungen an VAF geschlossen halten?"** Wenn ja,
> brauchst du eine kommerzielle Lizenz. Wenn nein, genügt die AGPL.
...
- **Rechtssicherheit** durch eine klar verhandelte Lizenz statt Auslegung des
  Copyleft-Umfangs.
- Optionale **Gewährleistungs- und Freistellungsregelungen** (unter der „as-is"-AGPL,
  Abschnitte 15–16, nicht verfügbar).
...
Kommerzielle Lizenzen werden **auf Anfrage** und passend zum Anwendungsfall angeboten
(Teamgröße, Deployment, Support-Stufe). Für ein Angebot oder Fragen:

- E-Mail an **legal@veyllo.io** mit einer kurzen Beschreibung des geplanten Einsatzes.
- Oder besuche **https://veyllo.io**.
