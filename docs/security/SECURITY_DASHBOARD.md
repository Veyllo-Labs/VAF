...
| `contact_access_changed` | A contact's "can reach your assistant" flag was switched, so the contact may (or may no longer) talk to the agent's Front Office. `username` who changed it, `path` the contact id, `detail` the contact and the new state; a bridge enrolling a new sender because Front Office is switched on for the channel writes it with `detail` "granted by the open Front Office: <name>" and `channel` the platform. Editing the endpoints of an already allowed contact is not an event, and the bulk grant of a channel's contacts when Front Office is switched on rides on that `front_office_changed` event as a count | `vaf/api/contact_routes.py` (create, patch), `whatsapp_bridge.py` (enrolment, inline) and `vaf/core/contacts_store.py` (`admit_front_office_sender`, the enrolment the Telegram and Discord bridges share) |
| `front_office_changed` | Front Office was switched on or off for a channel in Settings, WhatsApp, Telegram, Discord or mail alike: on, everyone who writes there is answered in Front Office mode and the channel's contacts in the book are allowed. `channel` the platform (`email` for mail), `username` who switched it, `detail` the new state (`on, N contacts granted` or `off`). The mail channel writes the same kind when its reply mode is changed, `detail` `reply mode draft` or `reply mode send`. One event per channel that really changed; a write that changes nothing records nothing | `vaf/api/front_office_routes.py` (the switch and the mail reply mode in Settings, Connections) |
