...
**Measured, and deliberately not fixed here:** nine production sites hand-roll "retrieve memory snippets and hand them to a prompt" (headless runner, gateway, automation, thinking mode, mail routes, voice agent, the search tool, `rag.py` itself and `context_tools`), five of them repeating the same `memory_rag_k` clamp, and the headless runner splices seven further ad-hoc context blocks inline. That is the measurement a future "one context contributor" primitive would rest on. Cross Chat Hint adds no copy of any of them, so it did not earn the refactor - the number is written here so the next round starts from it.
