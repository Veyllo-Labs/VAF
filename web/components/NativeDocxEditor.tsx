  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-0.5 border-b border-gray-200 bg-gray-50 px-2 py-1 shrink-0">
        ...
        <input value={selRun?.font_name || 'Arial'} disabled={!selectedParagraph} onChange={(e) => setSelRun((r) => ({ ...r, font_name: e.target.value }), `font-name:${selectedBlock?.kind ?? ''}:${selectedBlock?.sectionIndex ?? ''}:${selectedBlock?.kind === 'block' ? selectedBlock.blockIndex : (selectedBlock?.paragraphIndex ?? '')}`)} className="h-7 w-24 rounded border border-gray-200 bg-white px-1.5 text-xs text-gray-700 focus:outline-none disabled:opacity-40" placeholder="Font" title="Font family" />
        <Sep />
        ...
        <button type="button" onClick={exportPdf} disabled={isExportingPdf} className="flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-medium text-white bg-blue-500 hover:bg-blue-600 disabled:opacity-50 dark:bg-[#e6e6e6] dark:text-[#181818] dark:hover:bg-[#f5f5f5] dark:shadow-none" title="Save as PDF">
