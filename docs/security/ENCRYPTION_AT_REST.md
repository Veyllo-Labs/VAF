...
| The WhatsApp login | `~/.vaf/users/<name>/whatsapp/` | Baileys' Signal-protocol state, one file per key: `creds.json` with the identity key, pre-keys, sessions, sender keys, app-state keys | Owner-only file modes, not encryption: the bridge starts Node with umask 077 and `whatsapp_auth.harden_auth_dir` fixes older files (measured before: the directory 0755 and 379 of 381 files 0644). Encrypting them means giving `wa-bridge.js` an auth-state store of its own instead of Baileys' `useMultiFileAuthState`, which reads and writes these files directly |
