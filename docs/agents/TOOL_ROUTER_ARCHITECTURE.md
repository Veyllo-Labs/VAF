...
**Result truncation and the deliverable exemption:** the dispatch funnel caps every result (`max_result_chars`, 2000 in the chat lane) to protect the model's context from unbounded output. The cut leaves out the MIDDLE (`clip_middle` in `vaf/core/tool_dispatch.py`: six tenths of the budget for the start, the rest for the end): a head-only cut threw away exactly what sits at the end of a command's output, `host_bash`'s `Exit <code>` and a build's error; the history prune (`ContextManager.process_tool_output`) and `host_bash`'s own stream caps use the same helper. A tool whose result IS the deliverable declares `result_is_deliverable = True` on its class and is returned to the model whole, promising in exchange to keep its own output bounded (`use_skill`, `read_skill`, `room_invite` - all three measured in one live incident where the cap cut a skill body mid-instruction and an invitation briefing inside a block the result text orders the model to pass on "unchanged and complete" - and `read_file`, whose windowed reads the cap reduced to blind 2,000-char cuts with the line count and the start_line continuation facts destroyed; its `_cap_result` ceiling is the bounded-output promise, pinned by `tests/test_read_file_window.py`). The event stream stays capped independently (`event_result`), so observation never inherits the exemption.

The declaration travels the WHOLE path, not just the funnel - exempting only the funnel was measured insufficient (the seamless-compression stage became the new cut at 1500 chars, and the whole-content error belts painted the now-visible briefing vocabulary as a failure). Three further consumers ask the same resolver (`Agent._tool_is_deliverable`): `ContextManager.process_tool_output` never prunes a deliverable before it enters history, `tool_result_is_error(..., content_carrying=True)` judges it by the position-anchored belts only (step chip, timeline status), and `summarize_tool_turn(content_tools=...)` does the same for later turns' summaries. Guards: `tests/test_deliverable_results.py`, `tests/test_tool_result_error_detector.py`.
...
- **Every lane that uses the funnel.** Chat, the workflow engine, the librarian, the training runner, and any tool an embedder registered through `add_tool()`. The coder's inner tools too (`_coder_funnel`), except the two calls it answers inline (`web_fetch`, `web_deep_search`), which run no tool object. `multi_tool_use.parallel` returns before the funnel, so its inner tool calls are logged individually and the wrapper itself is not.
...
| `admin_only` | `bool` | `False` | When `True`, the tool is **hard-blocked** for non-admin users in the shared dispatch pipeline (`vaf/core/tool_dispatch.py`) and in the coder's own dispatch (`_coder_dispatch_refusal` in `vaf/tools/coder.py`), so it applies to every caller and not only to chat. The check uses the caller's role and scope; in a chat turn those are `_current_user_role` and `_current_user_scope_id`, set on the agent before each turn. This is a role-based check - distinct from `channel_restrictions` which is source-based. |
...
workflow step floor over each tool's own budget, the declared self-supervised tools minus `browser_agent`, its own `stop_check`,
...
gate (deliberate: a workflow runs unattended and a dialog would stall it, so gated tools
such as `host_bash` run in a workflow step without being asked - including in a
`run_temp` workflow the model builds itself; with the gate off, an authorizer's `ask()`
degrades to no opinion here, only `deny()` binds), `tool_start`/`tool_end` events (the lane reports
...
0. **Arguments that are not a JSON object are refused.** `execute` also takes the arguments as the JSON text the model sent; text that is not an object (a string value left unquoted, a bare list) is answered with `Tool Error: the arguments of this '<tool>' call are not valid JSON (...), so nothing was run` right after the audit line, before any other stage, and the tool never runs. `decode_arguments` in `vaf/core/tool_dispatch.py` is the one decoder, and the chat loop, the librarian and the coder all use it: each used to fall back to `{}`, so a tool whose parameters are all optional ran as a no-op and reported success (live incident: `update_working_memory` answered "Working Memory updated." while the plan it carried was dropped). The prefix is `Tool Error:` and not `Error:` on purpose: the chat loop answers a result starting with "error" with "do not retry", and this is the one failure a retry fixes. Guard: `tests/test_tool_arguments_decode.py`.
...
2a. **The account allowlist** - which tools this ACCOUNT may use, answered by the process-wide resolver the application registered with `set_account_allowlist_resolver` (the harness registers its auth-DB resolver, `vaf/auth/permissions.py`, at `vaf/main.py` import). Scopeless callers and admins are exempt and never reach the resolver; nothing registered means unrestricted; a RAISING registered resolver refuses (fail-closed, the authorizer's polarity - the harness resolver catches its own DB errors and stays fail-open internally). A blocked tool ends with `Security Error: ...` and **emits nothing**. Sits after the hard blocks and BEFORE the authorizer, so an account-level ban cannot be lifted by an `allow()`. Guard: `tests/test_tool_account_allowlist.py`, wiring: `tests/test_account_allowlist_wiring.py`.
    ...
    **The same answer, before the click.** Enforcing only at dispatch meant a restricted account was still *offered* what it could not run: the `/` suggestion list and the sub-agent hotbar are built from the `tools_list` the web server pushes, and that list was the full registry (custom-tool visibility was filtered, the account allowlist was not). The read side is `account_allows_tool(name, scope, role)` in `vaf/core/tool_dispatch.py` - exported on the facade, because the EXEMPTIONS are part of the answer and a surface that rebuilt the check from the resolver alone would strip an admin's own tools. All three `tools_list` producers in `vaf/core/web_server.py` (the `get_tools` handler, the post-mutation broadcast, the on-connect push) now filter through it and fail CLOSED per entry: a crashed resolver drops the entry rather than widening the menu. Guard: `tests/test_tool_list_respects_account.py`.
...
3. **Confirmation gate** - if `permission_level == "dangerous"`, the caller is asked (once / this chat / always / cancel). "Once" runs this call and remembers nothing (a tool that re-checks on its own, `python_exec`, hears about it through `accepts_call_confirmation` / the assigned `_call_confirmed`); "this chat" keeps the tool allowed for this person in this chat, in memory (`vaf/core/trust.py`, keyed on scope and session); "always" persists the tool policy and trusts the directory, per person. `side_effect_class == "irreversible"` adds a warning line. `permission_level == "system"` bypasses the gate entirely. A standing grant (`policy == "allow"`, or a trusted directory) runs the tool **silently** - no gate event at all; a chat grant runs it with a `gate_bypassed` event (`why="chat_grant"`). Both halves of that sentence are conditional on there being no authorizer: `req.ask()` gates a tool of ANY permission level and ignores standing grants, while `req.allow()` skips the gate for a `dangerous` tool without writing anything durable. When nobody can answer (headless, or an embedder that passed no asker) the call is refused with a string rather than blocking on a person who is not there.
4. **`tool_start` event.**
...
- After the policy check and **before** the confirmation gate, so a refusal cannot be replaced by a gate prompt: the **plan gate** (`write`/`dangerous` tools except `python_sandbox` need a plan in working memory first, answered with `[PLAN REQUIRED]`; skipped for sub-agents and non-interactive runs; the turn block's plan line reads the same state, `Agent._plan_gate_state` - see [CONTEXT_MANAGEMENT.md](../memory/CONTEXT_MANAGEMENT.md)), the **working-memory note firewall**, the **proactive-reply mutation gate** (`[CONFIRM REQUIRED]` for stored-state mutations and destructive delegation while the turn is a pickup of a reply that is not a clear affirmative; kill-switch `proactive_reply_mutation_gate_enabled`) and the **ask-first gate** (`[AWAITING USER]` for new `write`/`dangerous` tools and delegations on synthetic drain turns while a blocking question is pending; kill-switch `ask_first_drain_gate_enabled`).
- Before dispatch: **session plumbing** (`_session_id`, `_session_workspace`, `_agent` and the other per-tool runtime kwargs - these are plumbing, not identity, and are not part of the `identity_kwargs` declaration) and the **anti-re-delegation guard**. NAMED BOUNDARY: `write_file`, `document_writer` and `python_sandbox` keep `_session_id` by name although `session_id` can be declared. For them the chat decides WHERE files land (relative paths in the chat workspace, exports into it), and a declaration would hand the chat to every lane that has one, the coder included, whose relative paths belong to its project.
