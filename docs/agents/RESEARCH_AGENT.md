...
During a run the agent emits `research_state` events (published through `StatePublisher("research_state", min_interval=0.4, dedupe=True)` in `vaf/core/progress.py`, constructed per run; `force=True` on stage transitions bypasses the 0.4s window and never the duplicate check; telemetry `research_state_emitted`). The session id is read from `VAF_SESSION_ID` **only**, deliberately without the IPC fallback the coder and document agents use: that id is a process global, and adopting it here would start emitting one user's topic and sources into whichever session dispatched last: topic, stage, outline with per-section status (`planned/searching/writing/done/error`) and word progress, finished section HTML, sources (title/domain) and loop count. The SubAgent window renders a paper-style document viewer (the newest section types out, then swaps to rendered HTML), outline progress, clickable source citations, an activity feed and a status bar. Payload details: `docs/web-ui/WEBUI_WEBSOCKET_FLOW.md`.
