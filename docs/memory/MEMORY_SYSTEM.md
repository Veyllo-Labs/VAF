...
- **Scope:** Learning is per user (`user_scope_id`) and, inside a messenger chat with a contact, per chat: the main user's own conversations (Web UI and their own channel chats) compact into the general lane; a chat the agent answers for a contact (Front Office contact, open conversation) compacts into that chat's own namespace, which the general lane never reads (see [Chat memory namespaces](#chat-memory-namespaces-what-a-contact-chat-teaches-stays-in-that-chat)). Telegram relay contacts get no agent answer and no learning, and messages the bridges only store never reach a session.
