...
def test_resolve_picks_newest_stable_when_excluding_pre(monkeypatch):
    ...
    rel, why = uc.resolve_latest_release(include_prereleases=False)
    assert why is None
...
def test_resolve_includes_prerelease_when_eligible(monkeypatch):
    ...
    rel, why = uc.resolve_latest_release(include_prereleases=True)
    assert why is None
...
def test_resolve_non_200_returns_none(monkeypatch):
    ...
    assert uc.resolve_latest_release(False) == (None, "http:404")
...
def test_resolve_offline_returns_none(monkeypatch):
    ...
    def boom(*a, **k):
        raise uc.requests.ConnectionError("no network")
    monkeypatch.setattr(uc.requests, "get", boom)
    assert uc.resolve_latest_release(False) == (None, "offline")


# --- prerelease eligibility -----------------------------------------------------------------------

def test_eligible_explicit_arg_wins():
    assert uc.eligible_prereleases(True) is True
    assert uc.eligible_prereleases(False) is False
...
def test_eligible_auto_alpha_install_tracks_prereleases(monkeypatch):
    ...
    monkeypatch.setattr(uc, "__version__", "2.6.0a0")
    assert uc.eligible_prereleases() is True
...
def test_eligible_auto_stable_install_is_stable_only(monkeypatch):
    ...
    monkeypatch.setattr(uc, "__version__", "2.6.0")
    assert uc.eligible_prereleases() is False
...
def test_eligible_config_override_beats_auto(monkeypatch):
    ...
    monkeypatch.setattr(uc, "__version__", "2.6.0a0")                          # even on an alpha build
    assert uc.eligible_prereleases() is False
...
def test_compare_versions():
    assert uc.compare_versions("2.6.0a0", "2.6.0") < 0   # prerelease < final
    assert uc.compare_versions("2.6.0", "2.6.0") == 0
    assert uc.compare_versions("2.7.0", "2.6.0") > 0
