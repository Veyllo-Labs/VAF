...
| `tool_end` | `tool`, `duration_ms`, `ok`, `result` | normally closes every `tool_start`, including on tool error (the error is returned as the result string); rare early-return branches can leave a `tool_start` unclosed, so do not block forever waiting for the pair. `ok` is DISPATCH-level (False = the tool raised or the name was unknown), not the semantic success of the tool's output. `result` is always a string (non-string returns are stringified, `None` becomes `""`), capped at 800 characters with a `\n[…+N chars]` suffix (a literal U+2026 ellipsis, not three ASCII dots) carrying the cut length, and surrogate-scrubbed so the event survives `json.dumps`. The cap is independent of the `max_result_chars` the MODEL sees - that one may legitimately be unbounded, observation may not. On the error path the message IS the result |
| `gate_required` | `tool`, `cwd`, `reason`, `args_preview` (max 300 chars), `args_preview_truncated`, `args_preview_neutralized`, `args_preview_redacted`, `command_categories` (shell tools only) | the confirmation gate was reached. The preview is HARDENED, because a dialog is a control only while what it shows equals what will run: hidden and direction-changing characters become visible `[U+XXXX]` markers, credential material becomes `[redacted]` (the surrounding syntax is kept so the reader still sees what was passed), and a cut ends in `... [cut]` INSIDE the 300 characters. The three counters say what had to be changed; `command_categories` carries the offline classifier's verdict (`vaf/core/command_policy.py`). Two causes: a tool whose `permission_level` is `dangerous`, or an embedder's authorizer calling `req.ask()` - which gates a tool of any level and ignores a standing grant, and whose message becomes `reason`. In non-interactive mode the tool then returns an `[ERROR] ... requires confirmation ...` string and **no** `gate_decision` follows |
| `gate_decision` | `tool`, `decision`: `allow_once` \| `allow_chat` \| `allow_always` \| `cancel` | interactive runs only. An `allow_chat` answer given with no session to key on is reported as `allow_once`, because that is all it grants |
| `gate_bypassed` | `tool`, `cwd`, `reason`, `why`: `admin_bypass` \| `user_grant` \| `chat_grant` | a confirmation dialog was skipped by a standing hands-off switch or grant: `admin_bypass` is the machine-wide admin switch (`tool_confirmation_bypass_admins`), `user_grant` is the per-user grant the admin made in the user record (`permissions["confirmation_bypass"]`), `chat_grant` is the person's own "for this chat" answer for this tool in this session. Emitted INSTEAD of `gate_required`, once per skipped dialog - hands-off never means unobserved. An authorizer's explicit `ask()` is never bypassed |
...
- No sub-agent progress, deliberately rather than by omission. In the shipped
  ...
  [agents/SUBAGENT_IPC.md](agents/SUBAGENT_IPC.md). Three of the six sub-agents
  populate it (coder, document, learn); the other three have no denominator to
  report. The learn agent introduces NO new event types on this surface: its
  batch progress rides the IPC counts and the Web UI's `learn_state` frames
  (a live-view wire type, not an engine event), and its LLM calls go through
  `completion.complete` - covered by the same "in-process lanes emit no llm
  events" limitation above.
