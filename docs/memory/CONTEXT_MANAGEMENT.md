...
**Measured, and deliberately not fixed here:** eight production sites hand-roll "retrieve memory snippets and hand them to a prompt" (headless runner, automation, thinking mode, mail routes, voice agent, the search tool, `rag.py` itself and `context_tools`), four of them repeating the same `memory_rag_k` clamp (mail routes, headless runner, automation, thinking mode - `config.py` holds the default, which is not a repeat), and the headless runner splices seven further ad-hoc context blocks inline. That is the measurement a future "one context contributor" primitive would rest on. The counts were nine and five until the dead WebSocket gateway module was removed; it carried one of each, so the drop is a deletion, not a re-measurement. Cross Chat Hint adds no copy of any of them, so it did not earn the refactor - the number is written here so the next round starts from it.
...
- **Source**: the session files under `~/.vaf/sessions`, read at query time. Nothing is indexed, cached or written, which is also what makes the deleted-chat guarantee structural: deletion is an unlink, so a deleted chat is unreadable rather than filtered out.
