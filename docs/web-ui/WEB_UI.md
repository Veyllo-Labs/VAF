...
**Settings → Automations:** View scheduled automations (user-scoped when multi-user is used; root/global automations such as "Daily calendar check" are also shown so the list matches the agent's `list_automations` tool). To create one manually: click **Create New** (or use the **Kalender** entry in the sidebar footer) to open the calendar window; choose month, then day, then an hour slot in the automations lane. The window shows the appointments of the VAF calendar beside the automations: a day view with an all-day row, an events lane (a click on an empty hour creates an appointment; the popup edits title, time, all-day, location, description, linked contacts, the reminder and whether the event is also written into the connected calendar) and the automations lane (see [CALENDAR_INTEGRATION.md](../integrations/CALENDAR_INTEGRATION.md)). The sidebar footer also includes **Notifications** (opens the Notifications popup) and **Settings**. Opening the calendar window (footer) also triggers the calendar ensure-daily-check API when a calendar is connected, so the Daily calendar check appears in the list without opening Settings first. A popup lets you set repeat (once, daily, weekly, monthly, hourly), time, a detailed prompt, and an optional name. Creation is sent via WebSocket (`create_automation`); the list refreshes on success. The agent can also create automations via the `create_automation` tool in chat.
