...
1. **When**: Each chat entry (Web UI via headless runner, automation) runs a memory search on the **user message** before calling the agent. The result is passed as `memory_context` into `chat_step`.
