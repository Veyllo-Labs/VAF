...
1. **Front Office – Role and Rules** (bilingual DE/EN based on `user_language`)
   ...
   - **Owner notification (back-channel)**: The agent MUST notify the owner via their `main_messenger` when a contact has a request for the owner, gives an answer to a question the owner asked, shares important information, or asks something the agent cannot decide. The agent first replies to the contact, then calls the send tool of the owner's `main_messenger` (`send_telegram`, `send_whatsapp`, `send_discord` or `send_slack`; `send_whatsapp` reaches the owner unless a third-party number is named, the others only ever reach the owner; `send_to_user` is not available in Front Office) with a short, informative message (contact name + key content). Normal conversations (small talk, questions the agent can answer) do NOT trigger a notification.
...
6. **Owner notification (if applicable):** If the contact had a request, answer, or important info for the owner, the agent calls `send_telegram`, `send_whatsapp`, `send_discord` or `send_slack` (whichever is the owner's `main_messenger` from User Identity; `send_to_user` is not in the Front Office allow-list) to notify the owner with a short summary. `send_whatsapp` sends to the owner unless a third-party number is named; the others only reach the owner. A successful send to the owner is recorded on the contact's chat (`chat_marks.owner_asked_ts`, written by the agent after the dispatch): in the inbox that chat waits for the owner until they answer or the agent writes to the contact again (see [INBOX.md](../integrations/INBOX.md)).
