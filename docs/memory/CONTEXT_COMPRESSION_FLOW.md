...
- `memory_context` is passed into `chat_step(..., memory_context=None)` by the caller (headless runner or automation), which runs a RAG search with the **current user message** as the query. The CLI, the TUI and the embedding facade pass nothing, so there the block carries only its placeholder plus any cross-chat hints.
