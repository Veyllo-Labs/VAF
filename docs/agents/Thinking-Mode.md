...
- **Contacting the user (`ask_user`):** The agent reaches the user with the explicit **`ask_user`** tool - a clean, user-facing `message` (so chain-of-thought can no longer leak into the chat) + an optional `proposed_action` + optional `details` (the real content behind a teaser) + optional `options`, which a background run delivers as a numbered list under the message. The same tool asks the person in front of a CHAT too, where it ends the turn instead of tracking a request ([AGENT_LOOP.md](AGENT_LOOP.md)). The message is delivered to the user's configured **main channel** (Telegram/WhatsApp/Discord, or the Web UI if none) and **tracked as a request** - the agent does NOT pick the channel, `ask_user` routes it. If it stays unanswered on a messenger, it is escalated **once** to the Web UI with a note that it was already asked there. The run then **ends immediately** (`run_has_open_request` → break) - max one message per run, then it waits for the reply; continuing would leave the background run racing the main agent on the shared local model. The question is recorded in the transcript of the chat it was delivered to - the channel session (`telegram_<chat id>` and its siblings) for a messenger, the anchor web session for the Web UI - tagged `kind="thinking"`, through the one background-append primitive (`SessionManager.append_background_message`), so the main agent answering on that channel has asked the question in its own history and a live agent already on that session rebuilds from the file. The waiting latch adds the proposal and the findings on top when the user replies; it is not the record. It used to be: delivery opted out of recording so the question would not appear twice, and when a room wake consumed the latch the question was nowhere (live 2026-09-02).
- **Fallback delivery via `thinking_done(message=...)`:** Contacting the user must always go through a **tool call** - a question written as plain assistant text is never delivered (it would otherwise leak reasoning). Because a small local model sometimes composes the question but forgets to call `ask_user`, `thinking_done` accepts the same optional `message`/`proposed_action`/`source_note_id`/`source_todo_id`; both routes share one delivery path (`deliver_tracked_message`) that records the request, sets `waiting_for_reply`, and delivers via the user's main channel (`send_to_main_messenger`) - Telegram/WhatsApp/Discord, or the Web UI when no messenger is configured. A `run_has_open_request` guard prevents a second message when `ask_user` already raised one this run.
- **Outbound channel guard:** before the run starts, `_filter_thinking_send_tools()` removes **all** outbound send tools (an explicit name set, not a prefix match: `send_telegram`, `send_whatsapp`, `send_discord`, `send_slack`, `send_mail` and the channel-agnostic `send_to_user` - a new send tool must join `_SENT_TOOLS` or it survives as an untracked channel). The agent contacts the user only via `ask_user`, which routes the tracked message to the configured `main_messenger` (Telegram/WhatsApp/Discord) or the Web UI - so a raw send tool is never needed and cannot cause an untracked or duplicate send. `send_mail` is likewise never available: e-mail is not a `main_messenger` value, so a background run cannot send e-mail to an address it composed itself. The prompt carries the same rule, and the registry filter enforces it.
...
- **Workflow router off:** during a background run the workflow router does not run on the thinking prompt (`chat_step` gates it on `not thinking_mode`). Without this gate the router could match the run's own proactive prompt (which mentions e.g. "automatically at 7:00" / "create automation"), inject a `[WORKFLOW SUGGESTION]` nudge with mis-extracted variables, and steer the run toward an unintended "create a timer" proposal. A background run has no user request to route; it only does its own housekeeping.
...
- **Output:** Runs are logged to `logs/vaf_think_YYYY-MM-DD.log` (human-readable) and to JSON run logs. Every message sent to the user (question, nudge, escalation) is also recorded in the transcript of the chat it was delivered to, with its kind.
...
- **Outcome classification (next run):** the background run that owns the question classifies each
  ...
  asks ONE soft retrospective recap ("Hey, sorry - did we actually do that?"). This LLM-based
  decision replaces an earlier keyword match, which could not reliably classify replies that contain no
  explicit yes/no keyword.
  - **NOTE:** the WebUI message handler must NOT clear `waiting_for_reply` first - the main agent's
    `chat_step` needs it to build the reply context. Clearing it before `chat_step` runs would leave the
    main agent without that context.
  - **Reply context overrides stale state:** the injected reply note states that the user's message is a
    ...
    more prominent intent/plan block could dominate the prompt and the main agent could answer about the
    wrong topic.
...
- **Don't re-ask:** every run injects the requests raised in the last `thinking_recent_request_runs`
  ...
  same *topic* reworded - see [Semantic de-duplication](#proactive-intelligence-level-2).
...
| `thinking_max_tool_turns` | `15` | Hard cap on tool-result cycles per background turn (the main chat uses 75). Stops small models from tool-spinning in the background. |
...
When a run is aborted because the user became active (e.g. sent a message), the process does not simply stop. A short summary of the current run state (last tools used, last assistant message) is saved via `thinking_note_add` (e.g. *"Run interrupted (Turn 2). Last tools: list_automations. Last thought: …"*). The next run receives this note so the agent can continue from context instead of starting from scratch.
...
## Proactive intelligence (Level 2)
...
> clear floor (`tool_choice="required"` + a "silence is never the goal" mandate). When the desk is thin or
> empty, that mandate can pressure the model to invent a plausible fact just to satisfy it. The
...
- **The digest asks about the PERSON, so it leaves document text out.** Its queries are about routines,
  plans and interests, and a single learned PDF outnumbers every personal fact in the store: measured
  2026-08-30, these queries returned 6 of 8 chunks from a document, so half the "REAL MEMORIES about
  the user" a run was handed were PDF pages. The digest passes `exclude_documents=True`
  (`RagPipeline.search`), which filters in SQL and therefore keeps the result COUNT while changing what
  is in it (0 of 8 after). Ordinary RAG is unaffected - the flag is off everywhere else. See
  [MEMORY_SYSTEM.md](../memory/MEMORY_SYSTEM.md#retrieval-document-text-vs-facts-about-the-person).
...
- **Message gate (3 modes, set per turn in `deliver_tracked_message`):** a FREE message (no
  ...
  - **`off`** (gather / forced-resolution): a free message is **blocked** - this prevents the generic turn-0
    stock phrase ("no tasks, I'm ready when you need me") from being sent before the proactive
    flow runs.
  - **`grounded`** (proactive step): a **backstop** check - the suggestion is delivered only if `message` OR
    ...
    rest can still pass it. The real
  ...
  `message`/`details`, so the small model stops churning.
...
5. **Run:** `_run_thinking_for_user()` runs in a daemon thread:
   ...
   - **Level-0 ledger:** a deterministic snapshot of the open notes/todos is built at run start
   ...
   - **Content-driven ladder (not turn-count):** turn 0 gathers; from turn 1, each open note/todo is driven
     ...
     the prompt switches to optional proactive upkeep (Level 1). No "wrap up now / FINAL TURN" pressure.
   ...
   - **History Synchronization:** every message the run sends (`ask_user`, the `thinking_done` fallback, the nudge, the escalation) is appended to the transcript of the chat it was delivered to at the moment of delivery, tagged with its kind (`thinking` / `nudge`). There is no separate scan of the run history for send tools any more: the send tools are stripped before the run, so the scanner that used to do this could never fire.
...
- **A rung owns the tools its prompt names.** The relevance rung is told to check something with
  `web_search`; when the router did not offer it, the model spent a tool call on `search_tools` to go
  find it first (live 2026-08-30, the 16:30 run). `_THINKING_NODE_REQUIRED_TOOLS` pins what a rung was
  instructed to use, through the same cap and backstop as the exit tools. A prompt that names a tool the
  set does not carry costs a turn, and on a turn budget sized for the ladder that is the rung.
...
| `update_intent` | No-op | A background run must not overwrite the main chat's `user_intent`. The tool returns a no-op in thinking mode (the agent's own run kind, not the shared environment - a concurrent chat turn used to get the same no-op), so a run cannot write itself a directive that the next run would read back as a standing order |
...
> *"User confirmed Alice birthday automation is fully handled - do not mention it again"*
...
- [2026-02-20 14:30] User confirmed Alice birthday automation handled - do not ask again
...
This is no longer a keyword guess at reply time. A reply that asks a question back (e.g. "no, I didn't, why do you ask?")
is therefore never silently dropped as a decline: the LLM classifier weighs
...
1. **Centralized Sync (`vaf/core/agent.py`):** In `chat_step()`, the state is cleared with the full `user_reply_text`. This covers all input channels (Web, CLI, Messenger) - and ONLY a turn that is the user speaking in their own chat. The waiting latch is scope-keyed on disk, so every `chat_step` on that scope reads it: the runner's room and drain turns, an automation run, the thinking run itself. `Agent._turn_is_from_the_user` is the one test both "the user replied" latches use (this one and the ask-first latch): a synthetic drain turn, a queue turn the harness marked as not a person (timer, automation), and a background run's own prompt (`run_kind` thinking/automation) neither record a reply nor clear the state. Live 2026-09-02: an A2A room wake ran through the pickup, the room prompt was recorded as the user's reply, the latch was cleared, and the real answer on Telegram an hour later met an agent that had never asked.
...
[THINKING RUN] 2026-02-20T14:30:45.123
  ...
  [tool] (completed)
  [assistant] I checked your todos...
```
...
| Level-0 ledger & gate | `vaf/core/thinking_ledger.py` | `build_ledger()`, `item_resolved()`, `unresolved_items()`, `build_gate_nudge()` - the completion-gate logic (pure, unit-testable) |
...
| Transcript record | `vaf/core/session.py` · `vaf/core/messaging_connections.py` | `SessionManager.append_background_message()` - the one way a background lane writes into a chat (question, nudge, escalation, automation result); `send_to_main_messenger(kind=...)` mirrors a messenger delivery into the channel session through it |
| Who counts as a reply | `vaf/core/agent.py` | `_turn_is_from_the_user()` - the one test both latches read; a room wake, a timer, an automation and a background run's own prompt fail it |
