...
- **Veyllo** - First-party Veyllo API. `veyllo-chat` is multimodal, a single model for both text chat and image input (vision); the same key also serves speech-to-text via `veyllo-transcribe` (set `speech_stt_provider = veyllo`), which becomes the default STT lane the first time a Veyllo key is added, at onboarding or later, when no STT provider was chosen (local fallback always applies)
...
6. When the model needs more than the description covers (exact colours, positions, small text, finding an object), it calls the **`analyze_image`** tool, which re-reads the image - live agent history first, then the persisted session - and runs a fresh, targeted `vision_infer()` pass, returning text. It also looks at image FILES the agent made or found (`image_path`): a relative path means the chat workspace, an absolute one any file the person may read. The tool declares `file_access = "read"`, so it is held to the same per-user read jail as `read_file`; the vision model can describe nothing the caller could not already read. `read_file` on an image returns a pointer to `analyze_image` with the image's path, never its bytes.
...
`vision_infer()` is the single choke point every image-to-text lane goes through - the base description and `analyze_image`, PDF OCR, and the browser agent (page descriptions and CAPTCHA tile reads; a stalled run on a vision-capable model is shown the screenshot directly instead). It never raises and filters provider error sentinels (`[API Error from …]`) and reasoning `<think>`/control-JSON chunks, so a failed or noisy vision call never becomes a stored description (it returns `None`, leaving the image to be re-described or inspected via `analyze_image` later).
