...
Both paths ask what to call your agent; leave the question unanswered on either and it
takes a generated suggestion, something like `Nobel4831SkyBlue`.
...
auth, and then runs a five-step progress bar: **Admin → Name → Soul → Veyllo API → 2FA**.
...
### 2. Agent name
The same question the terminal setup asks: what do you want to call your agent? First the
agent gets its name, then its character. The field offers a generated suggestion (the
shape the terminal setup suggests, minted by the server so the two never drift); leaving
it empty takes the suggestion, and the dice button rolls a new one. The name lands in the
agent's workspace identity (`~/.vaf/users/<admin>/identity.json`), which the system
prompt reads - it is how the agent introduces itself in chat and in agent rooms, and it
can be changed later in Settings.

### 3. Soul (agent personality)
...
The only provider the wizard offers is **Veyllo** (the optional step 4). If you skip it, VAF
...
- a few identity keys in `~/.vaf/config.json` (e.g. the admin scope id and username, and
  ...
  entered a key in step 4;
- your agent's `soul.md` and identity files under `~/.vaf/users/<admin>/`.

After step 5 you land in the main web UI, logged in, with your agent live.
