...
| `outward_send_hold` | `True` | An outward message the agent prepares on the person's own web chat turn waits for them: a mail is parked as a draft, a WhatsApp message with an explicit number as a parked call, both shown as a card to send, edit or discard, and the chat turn ends at the draft. Only calls that can reach somebody else are held, and only on the web chat; automations, workflows, channel turns and Front Office answers send as before. Admin-only (`GLOBAL_CONFIG_KEYS`): there is one config file, so switching the hold off is a decision for every account on the machine, and a guard its own subject can disable is not a guard. |
...
| `telegram_config` | `None` | Telegram bot config (set via UI): `enabled`, `verified`, `bot_username`, the whitelists. The bot token is NOT kept here: a `bot_token` written into this block (by hand or by an older release) moves into the encrypted key ring on the next read, and the config API blanks it in what a browser receives (`vaf/core/channel_secrets.py`). |
| `telegram_debounce_seconds` | `5` | Telegram message debounce. |
| `telegram_idle_timeout` | `120` | Telegram session idle timeout. |
| `whatsapp_config` | `None` | WhatsApp bridge config (set via UI). Sub-keys: `enabled`; `inbound_to_agent` (`True`; off stops every sender before the agent and closes the Front Office switch on WhatsApp with it, read in one place, `messaging_connections.whatsapp_inbound_to_agent`); `reply_window_hours` (`72`, `0` = off) which is DISPLAY ONLY: it says how long a conversation the agent started counts as live in the WhatsApp window and admits nobody, because a window that admitted the people the agent had written to answered them on channels the owner had closed; `chat_sync_interval_sec` (`600`); `whitelist` (registered main-user numbers, one per VAF account: `phone_number`, `user_scope_id`, `vaf_username`; the linked account itself is the agent's number and is refused here); managed: `lid_to_e164`, `owner_control`, `chat_activity`. See [WHATSAPP_INTEGRATION.md](../integrations/WHATSAPP_INTEGRATION.md). |
...
| `mail_auto_reply_max_per_address_per_hour` | `3` | Admin-only. At most this many automatic mail answers to one address per hour; the rest of that address's mail waits for the owner (the autoresponder loop guard, independent of headers). `0` means no automatic answer at all: every mail waits for the owner. |
| `mail_auto_reply_max_per_address_per_day` | `10` | Admin-only. The same cap per day (`0` again means no automatic answer); reaching it writes `mail_auto_reply_capped` to the security log once per address and day. |
...
| `mail_composer_enabled` | `True` | Offer the Composer (draft / rewrite) in the compose box of every window that has one; named for the mail window, which had it first, and governing the messenger windows through the same shared lane (`vaf/core/composer_lane.py`). The lane is inert until a user clicks it, makes exactly one model call with NO tools, and only ever fills the textarea - it can never send. Admin-only. |
...
| `mail_composer_max_output_tokens` | `2500` | Output cap for the Composer's single model call. On a thinking provider (Veyllo, DeepSeek) the model's reasoning counts against this cap as well, and a cap the reasoning alone exhausts leaves no reply, so it is sized well above the length of a message. Admin-only. |
...
| `channel_ingress_policy` | `{...}` | Inbound-channel pairing policy: `mode` (`paired_only`, the only value; it is the floor, not a choice), `throttle_seconds` (`60`) for the REJECT log lines, and per channel (`telegram`, `whatsapp`, `discord`, `email`) a `mode` (`inherit`) and `open_to_new_senders` (`False`); the `email` entry additionally carries `reply_mode` (`draft`: the agent's answer is held in the mail outbox for the owner's approval; `send`: it leaves at once) and `opened_at` (the moment the mail switch was turned on, stamped by the switch; mail dated before it is never answered). `open_to_new_senders` is the Front Office switch of that channel in Settings → Connections: it answers everybody the owner has not decided about, and only there (inert outside WhatsApp, Telegram, Discord and mail, and on mail only a sender the provider verified is answered, see [FRONT_OFFICE.md](../agents/FRONT_OFFICE.md#mail)). Who else gets in is the person's own state in the contact book (`assistant_access`), not a key here: the two expert doors that used to say the same thing (`permissive`, `allow_contact_fallback`) are gone, a stored `permissive` is read once and coerced to `paired_only`, and an open channel is what the security overview warns about now (`front_office_state` and `set_front_office` in `vaf/core/channel_ingress_policy.py`). Admin-only. |
| `connection_enabled_by_scope` | `None` | Per-scope connection toggles: `{<scope>: {<channel>: bool}}`, each account's own switch. Read by one rule, `messaging_connections.channel_enabled_for_scope`: on WhatsApp every account but the local admin (whose switch is `whatsapp_config.enabled`) needs its own; on Telegram nothing is on while the bot is off, admins ride the bot and every other account needs its own; Discord has no lane but the local admin's. A pairing by code turns the account's Telegram switch on. |
