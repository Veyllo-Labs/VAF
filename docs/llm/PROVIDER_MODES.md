...
| Voice call lane (live call) | The live-call voice agent rides the main provider. On `provider=local` it **time-shares** the single llama server (`voice_agent.is_exclusive()` is True): availability is an honest probe of `GET :8080/v1/models` (in-process library mode cannot serve the call lane), a reply is one non-streaming `POST /v1/chat/completions` with `chat_template_kwargs: {enable_thinking: false}` (a local reasoning model otherwise burns the whole voice token budget on `reasoning_content` and nothing is spoken; templates without the variable ignore it), and voice turns pause while the main agent runs a delegated task (the frontend mutes the call; a server belt answers `voice_call_error "busy_local"` before STT) - never two concurrent inferences. API providers stream via `APIBackendManager` as before. | `vaf/core/voice_agent.py` (`_resolve_backend`, `is_exclusive`, `available`, `_local_chat`), `web_server.py` (`voice_call_started` `exclusive` flag, busy belt) | [VOICE_AGENT.md](../agents/VOICE_AGENT.md) |
