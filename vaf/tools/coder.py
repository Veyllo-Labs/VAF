...
- `write_file(path, content)`: Create/update files - YOU MUST CALL THIS to actually create code. For BINARY deliverables (png/jpg/pdf) render in `python_sandbox` to a relative path and pass `export_files=['<name>.png']` in the same call - never write script source into an image-named file.
...
- `write_file(path, content)` - Create/modify files with actual code (binary artifacts: python_sandbox with export_files)
                        ...
                        if fn_name == "python_sandbox":
                            ...
                            if _sandbox_writes_files:
                                result = (
                                    ...
                                    "sandbox to a RELATIVE path and pass export_files=['<name>.png'] in the SAME "
                                    "python_sandbox call — the file is copied to the chat workspace after the "
                                    "run. Never write script source into a .png-named file, and never print "
                                    "base64 into context (large files arrive truncated).\n"
