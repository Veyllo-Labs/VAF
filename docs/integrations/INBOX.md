...
| `mode` | which lane answers, the same rule the bridge applies (`inbox.chat_mode`): `owner`, `contact` (a person the owner allowed, or anybody while that channel's Inbound is open; on WhatsApp the switch counts only while `inbound_to_agent` forwards at all, and on Telegram only with exactly one owner on the bot, the two things `messaging_connections.front_office_open` folds into one answer), `readonly` (a person the owner blocked, and everybody else under a closed channel), `needs_assign` (an unresolved WhatsApp `@lid`), `admin` (the paired Discord admin's direct message), `relay` (Telegram), `mail`, `room`. There is no `conversation` any more: the reply window decided who was answered, and it decides nothing now |
...
lane's reach: up to 200 primary threads from the newest 1000, read in pages of 200; one mail service for the call, one flags op per message for the writeback as
the mail window's own read marking does), and the person's
...
window widens the list again (channel and view reset; the search, the selection and the
phone's pane too): a narrowing is for one look. A response that is not the newest
